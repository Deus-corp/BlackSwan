...
Output as JSON list of objects with keys: "path" (string), "description" (string), "code_skeleton" (string with Python code skeleton).
"""
        text = await self._generate_with_retry(prompt)
        if text:
            try:
                proposals = json.loads(re.sub(r'```json|```', '', text).strip())
