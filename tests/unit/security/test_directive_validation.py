...
def _real_execution_sandbox_materialization_preflight_scaffold(**overrides):
    item = {
        "type": "replay_lifecycle_retry_real_execution_sandbox_materialization_preflight_scaffold",
        "real_execution_sandbox_materialization_preflight_scaffold_id": (
            "sandbox-materialization-preflight-1"
        ),
        "real_execution_sandbox_request_envelope_scaffold_id": "sandbox-envelope-1",
        "real_execution_sandbox_adapter_request_preflight_id": "sandbox-preflight-1",
        "real_execution_sandbox_adapter_scaffold_id": "sandbox-scaffold-1",
        ...
        "source_envelope_scaffold_exists": True,
        "source_envelope_scaffold_fail_closed": True,
        "source_envelope_scaffold_deny_by_default": True,
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_input_strategy": "explicit_allowlist_only",
        "sandbox_output_strategy": "explicit_allowlist_only",
        "sandbox_rollback_strategy": "workspace_destruction",
        ...
        "production_paths_mutated": False,
        "production_secrets_accessed": False,
        "source_envelope_generation_enabled": False,
        "source_envelope_materialized": False,
        "source_envelope_executable": False,
        "source_workspace_creation_enabled": False,
        "source_input_materialization_enabled": False,
        "source_command_rendering_enabled": False,
        "source_sandbox_execution_enabled": False,
        "source_result_generation_enabled": False,
        "source_execution_performed": False,
        "source_subprocess_invoked": False,
        "source_real_execution_enabled": False,
        "source_external_side_effects_performed": False,
        "source_production_paths_mutated": False,
        "source_production_secrets_accessed": False,
        "recommended_next_action": (
...
def test_validate_retry_real_execution_sandbox_materialization_preflight_scaffold_accepts_blocked_fail_closed() -> None:
    result = (
        validate_replay_lifecycle_retry_real_execution_sandbox_materialization_preflight_scaffold(
            _real_execution_sandbox_materialization_preflight_scaffold()
        )
    )
    ...
    assert result["severity"] == "info"
    assert result["reasons"] == []
    assert result["sandbox_materialization_preflight_scaffold_status"] == "blocked"
    assert result["sandbox_materialization_preflight_scaffold_fail_closed"] is True
    assert result["sandbox_materialization_preflight_scaffold_deny_by_default"] is True
    assert result["sandbox_materialization_preflight_enabled"] is False
    assert result["sandbox_materialization_preflight_passed"] is False
    assert result["sandbox_request_envelope_generation_enabled"] is False
    assert result["sandbox_request_envelope_materialized"] is False
    assert result["sandbox_request_envelope_executable"] is False
    assert result["sandbox_workspace_creation_enabled"] is False
    assert result["sandbox_input_materialization_enabled"] is False
    assert result["sandbox_execution_enabled"] is False
    assert result["execution_performed"] is False
    assert result["subprocess_invoked"] is False
    assert result["real_execution_enabled"] is False
...
def test_validate_retry_real_execution_sandbox_materialization_preflight_scaffold_rejects_preflight_enabled() -> None:
    record = _real_execution_sandbox_materialization_preflight_scaffold(
        sandbox_materialization_preflight_enabled=True
    )
    record["payload"]["sandbox_materialization_preflight_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_materialization_preflight_enabled_must_be_false" in result[
        "reasons"
    ]
    assert "payload_sandbox_materialization_preflight_enabled_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_materialization_preflight_scaffold_rejects_preflight_passed() -> None:
    record = _real_execution_sandbox_materialization_preflight_scaffold(
        sandbox_materialization_preflight_passed=True
    )
    record["payload"]["sandbox_materialization_preflight_passed"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_materialization_preflight_passed_must_be_false" in result[
        "reasons"
    ]
    assert "payload_sandbox_materialization_preflight_passed_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_materialization_preflight_scaffold_rejects_input_materialization_enabled() -> None:
    record = _real_execution_sandbox_materialization_preflight_scaffold(
        sandbox_input_materialization_enabled=True
    )
    record["payload"]["sandbox_input_materialization_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_input_materialization_enabled_must_be_false" in result[
        "reasons"
    ]
    assert "payload_sandbox_input_materialization_enabled_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_materialization_preflight_scaffold_rejects_sandbox_execution_enabled() -> None:
    record = _real_execution_sandbox_materialization_preflight_scaffold(
        sandbox_execution_enabled=True
    )
    record["payload"]["sandbox_execution_enabled"] = True
...
def _real_execution_sandbox_workspace_plan_scaffold(**overrides):
    item = {
        "type": "replay_lifecycle_retry_real_execution_sandbox_workspace_plan_scaffold",
        "real_execution_sandbox_workspace_plan_scaffold_id": (
            "sandbox-workspace-plan-1"
        ),
        "real_execution_sandbox_materialization_preflight_scaffold_id": (
            "sandbox-materialization-preflight-1"
        ),
        ...
        "proposal_id": "proposal-1",
        "rendered_command_id": "rendered-1",
        "schema_version": "real-execution-sandbox-workspace-plan-scaffold/v1",
        ...
        "source_materialization_preflight_scaffold_exists": True,
        "source_materialization_preflight_scaffold_fail_closed": True,
        "source_materialization_preflight_scaffold_deny_by_default": True,
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        ...
        "production_paths_mutated": False,
        "production_secrets_accessed": False,
        "source_materialization_preflight_enabled": False,
        "source_materialization_preflight_passed": False,
        "source_envelope_generation_enabled": False,
        "source_envelope_materialized": False,
        "source_envelope_executable": False,
        "source_workspace_creation_enabled": False,
...
def test_validate_retry_real_execution_sandbox_workspace_plan_scaffold_accepts_blocked_fail_closed() -> None:
    result = (
        validate_replay_lifecycle_retry_real_execution_sandbox_workspace_plan_scaffold(
            _real_execution_sandbox_workspace_plan_scaffold()
        )
    )
    ...
    assert result["severity"] == "info"
    assert result["reasons"] == []
    assert result["sandbox_workspace_plan_scaffold_status"] == "blocked"
    assert result["sandbox_workspace_plan_scaffold_fail_closed"] is True
    assert result["sandbox_workspace_plan_scaffold_deny_by_default"] is True
    assert result["sandbox_workspace_plan_generation_enabled"] is False
    assert result["sandbox_workspace_plan_materialized"] is False
    assert result["sandbox_workspace_plan_executable"] is False
    assert result["sandbox_workspace_directory_creation_enabled"] is False
    assert result["sandbox_workspace_created"] is False
    assert result["sandbox_workspace_cleanup_registered"] is False
    assert result["sandbox_workspace_creation_enabled"] is False
    assert result["sandbox_input_materialization_enabled"] is False
    assert result["sandbox_execution_enabled"] is False
    assert result["execution_performed"] is False
...
def test_validate_retry_real_execution_sandbox_workspace_plan_scaffold_rejects_plan_generation_enabled() -> None:
    record = _real_execution_sandbox_workspace_plan_scaffold(
        sandbox_workspace_plan_generation_enabled=True
    )
    record["payload"]["sandbox_workspace_plan_generation_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_workspace_plan_generation_enabled_must_be_false" in result[
        "reasons"
    ]
    assert "payload_sandbox_workspace_plan_generation_enabled_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_workspace_plan_scaffold_rejects_plan_executable() -> None:
    record = _real_execution_sandbox_workspace_plan_scaffold(
        sandbox_workspace_plan_executable=True
    )
    record["payload"]["sandbox_workspace_plan_executable"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_workspace_plan_executable_must_be_false" in result["reasons"]
    assert "payload_sandbox_workspace_plan_executable_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_workspace_plan_scaffold_rejects_workspace_created() -> None:
    record = _real_execution_sandbox_workspace_plan_scaffold(
        sandbox_workspace_created=True
    )
    record["payload"]["sandbox_workspace_created"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_workspace_created_must_be_false" in result["reasons"]
    assert "payload_sandbox_workspace_created_must_be_false" in result["reasons"]


def test_validate_retry_real_execution_sandbox_workspace_plan_scaffold_rejects_sandbox_execution_enabled() -> None:
    record = _real_execution_sandbox_workspace_plan_scaffold(
        sandbox_execution_enabled=True
    )
    record["payload"]["sandbox_execution_enabled"] = True
...
def _real_execution_sandbox_workspace_preparation_preflight_scaffold(**overrides):
    item = {
        "type": (
        ...
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_id": (
            "sandbox-workspace-preparation-preflight-1"
        ),
        "real_execution_sandbox_workspace_plan_scaffold_id": (
            "sandbox-workspace-plan-1"
        ),
        ...
        "sandbox_workspace_preparation_preflight_scaffold_exists": True,
        ...
        "sandbox_workspace_preparation_preflight_requires_workspace_plan": True,
        "sandbox_workspace_preparation_preflight_requires_materialization_preflight": True,
        "sandbox_workspace_preparation_preflight_requires_envelope_scaffold": True,
        "sandbox_workspace_preparation_preflight_requires_request_preflight": True,
        "sandbox_workspace_preparation_preflight_requires_policy_matrix": True,
        "sandbox_workspace_preparation_preflight_requires_known_capability": True,
        "sandbox_workspace_preparation_preflight_requires_known_policy": True,
        "sandbox_workspace_preparation_preflight_requires_operator_authorization": True,
        "sandbox_workspace_preparation_preflight_requires_approval_lineage": True,
        "sandbox_workspace_preparation_preflight_requires_final_gate": True,
        "sandbox_workspace_preparation_preflight_requires_dry_run_envelope": True,
        "sandbox_workspace_preparation_preflight_requires_rollback_plan": True,
        "sandbox_workspace_preparation_preflight_requires_post_execution_evidence": True,
        "sandbox_workspace_preparation_preflight_rejects_unknown_capability": True,
        "sandbox_workspace_preparation_preflight_rejects_unknown_policy": True,
        "sandbox_workspace_preparation_preflight_rejects_orphans": True,
        "sandbox_workspace_preparation_preflight_rejects_stale_records": True,
        "source_workspace_plan_scaffold_exists": True,
        "source_workspace_plan_scaffold_fail_closed": True,
        "source_workspace_plan_scaffold_deny_by_default": True,
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_workspace_plan_strategy": "explicit_plan_only",
        ...
        "production_paths_mutated": False,
        "production_secrets_accessed": False,
        "source_workspace_plan_generation_enabled": False,
        "source_workspace_plan_materialized": False,
        "source_workspace_plan_executable": False,
        "source_directory_creation_enabled": False,
        "source_workspace_created": False,
        "source_cleanup_registered": False,
        "source_materialization_preflight_enabled": False,
        "source_materialization_preflight_passed": False,
        "source_workspace_creation_enabled": False,
        "source_input_materialization_enabled": False,
...
def test_validate_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold_accepts_blocked_fail_closed() -> None:
    result = (
        validate_replay_lifecycle_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold(
            _real_execution_sandbox_workspace_preparation_preflight_scaffold()
        )
    )
    ...
    assert result["reasons"] == []
    assert (
        result["sandbox_workspace_preparation_preflight_scaffold_status"]
        == "blocked"
    )
    assert (
        result["sandbox_workspace_preparation_preflight_scaffold_fail_closed"]
        is True
    )
    assert (
    ...
    assert result["sandbox_workspace_preparation_preflight_enabled"] is False
    assert result["sandbox_workspace_preparation_preflight_passed"] is False
    assert result["sandbox_workspace_plan_generation_enabled"] is False
    assert result["sandbox_workspace_plan_executable"] is False
    assert result["sandbox_workspace_directory_creation_enabled"] is False
    assert result["sandbox_workspace_created"] is False
...
def test_validate_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold_rejects_preflight_enabled() -> None:
    record = _real_execution_sandbox_workspace_preparation_preflight_scaffold(
        sandbox_workspace_preparation_preflight_enabled=True
    )
    record["payload"]["sandbox_workspace_preparation_preflight_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_workspace_preparation_preflight_enabled_must_be_false" in result[
        "reasons"
    ]
    assert (
        "payload_sandbox_workspace_preparation_preflight_enabled_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold_rejects_preflight_passed() -> None:
    record = _real_execution_sandbox_workspace_preparation_preflight_scaffold(
        sandbox_workspace_preparation_preflight_passed=True
    )
    record["payload"]["sandbox_workspace_preparation_preflight_passed"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_workspace_preparation_preflight_passed_must_be_false" in result[
        "reasons"
    ]
    assert (
        "payload_sandbox_workspace_preparation_preflight_passed_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold_rejects_directory_creation_enabled() -> None:
    record = _real_execution_sandbox_workspace_preparation_preflight_scaffold(
        sandbox_workspace_directory_creation_enabled=True
    )
    record["payload"]["sandbox_workspace_directory_creation_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_workspace_directory_creation_enabled_must_be_false" in result[
        "reasons"
    ]
    assert (
        "payload_sandbox_workspace_directory_creation_enabled_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold_rejects_workspace_created() -> None:
    record = _real_execution_sandbox_workspace_preparation_preflight_scaffold(
        sandbox_workspace_created=True
    )
    record["payload"]["sandbox_workspace_created"] = True
...
def test_validate_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold_rejects_sandbox_execution_enabled() -> None:
    record = _real_execution_sandbox_workspace_preparation_preflight_scaffold(
        sandbox_execution_enabled=True
    )
    record["payload"]["sandbox_execution_enabled"] = True
...
def _real_execution_sandbox_input_materialization_plan_scaffold(**overrides):
    item = {
        "type": (
        ...
        "real_execution_sandbox_input_materialization_plan_scaffold_id": (
            "sandbox-input-materialization-plan-1"
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_id": (
            "sandbox-workspace-preparation-preflight-1"
        ),
        ...
        "source_workspace_preparation_preflight_scaffold_exists": True,
        "source_workspace_preparation_preflight_scaffold_fail_closed": True,
        "source_workspace_preparation_preflight_scaffold_deny_by_default": True,
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_workspace_plan_strategy": "explicit_plan_only",
        "sandbox_workspace_preparation_strategy": (
            "preflight_only_no_directory_creation"
        ...
        "production_paths_mutated": False,
        "production_secrets_accessed": False,
        "source_workspace_preparation_preflight_enabled": False,
        "source_workspace_preparation_preflight_passed": False,
        "source_directory_creation_enabled": False,
        "source_workspace_created": False,
        "source_cleanup_registered": False,
        "source_workspace_creation_enabled": False,
...
def test_validate_retry_real_execution_sandbox_input_materialization_plan_scaffold_accepts_blocked_fail_closed() -> None:
    result = (
        validate_replay_lifecycle_retry_real_execution_sandbox_input_materialization_plan_scaffold(
            _real_execution_sandbox_input_materialization_plan_scaffold()
        )
    )
    ...
    assert result["severity"] == "info"
    assert result["reasons"] == []
    assert result["sandbox_input_materialization_plan_scaffold_status"] == "blocked"
    assert result["sandbox_input_materialization_plan_scaffold_fail_closed"] is True
    assert result["sandbox_input_materialization_plan_scaffold_deny_by_default"] is True
    assert result["sandbox_input_materialization_plan_generation_enabled"] is False
    assert result["sandbox_input_materialization_plan_materialized"] is False
    assert result["sandbox_input_materialization_plan_executable"] is False
    assert result["sandbox_input_materialization_enabled"] is False
    assert result["sandbox_inputs_materialized"] is False
    assert result["sandbox_workspace_preparation_preflight_enabled"] is False
    assert result["sandbox_workspace_preparation_preflight_passed"] is False
    assert result["sandbox_workspace_directory_creation_enabled"] is False
    assert result["sandbox_workspace_created"] is False
    assert result["sandbox_command_rendering_enabled"] is False
    assert result["sandbox_execution_enabled"] is False
    assert result["execution_performed"] is False
    assert result["subprocess_invoked"] is False
    assert result["real_execution_enabled"] is False
...
def test_validate_retry_real_execution_sandbox_input_materialization_plan_scaffold_rejects_plan_generation_enabled() -> None:
    record = _real_execution_sandbox_input_materialization_plan_scaffold(
        sandbox_input_materialization_plan_generation_enabled=True
    )
    record["payload"]["sandbox_input_materialization_plan_generation_enabled"] = True
    ...
    assert result["severity"] == "critical"
    assert (
        "sandbox_input_materialization_plan_generation_enabled_must_be_false"
        in result["reasons"]
    )
    assert (
        "payload_sandbox_input_materialization_plan_generation_enabled_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_input_materialization_plan_scaffold_rejects_plan_executable() -> None:
    record = _real_execution_sandbox_input_materialization_plan_scaffold(
        sandbox_input_materialization_plan_executable=True
    )
    record["payload"]["sandbox_input_materialization_plan_executable"] = True
    ...
    assert result["severity"] == "critical"
    assert (
        "sandbox_input_materialization_plan_executable_must_be_false"
        in result["reasons"]
    )
    assert (
        "payload_sandbox_input_materialization_plan_executable_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_input_materialization_plan_scaffold_rejects_input_materialization_enabled() -> None:
    record = _real_execution_sandbox_input_materialization_plan_scaffold(
        sandbox_input_materialization_enabled=True
    )
    record["payload"]["sandbox_input_materialization_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_input_materialization_enabled_must_be_false" in result["reasons"]
    assert "payload_sandbox_input_materialization_enabled_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_input_materialization_plan_scaffold_rejects_inputs_materialized() -> None:
    record = _real_execution_sandbox_input_materialization_plan_scaffold(
        sandbox_inputs_materialized=True
    )
    record["payload"]["sandbox_inputs_materialized"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_inputs_materialized_must_be_false" in result["reasons"]
    assert "payload_sandbox_inputs_materialized_must_be_false" in result["reasons"]


def test_validate_retry_real_execution_sandbox_input_materialization_plan_scaffold_rejects_sandbox_execution_enabled() -> None:
    record = _real_execution_sandbox_input_materialization_plan_scaffold(
        sandbox_execution_enabled=True
    )
    record["payload"]["sandbox_execution_enabled"] = True
...
def _real_execution_sandbox_command_render_plan_scaffold(**overrides):
    item = {
        "type": (
        ...
        "sandbox_command_render_plan_scaffold_exists": True,
        "sandbox_command_render_plan_scaffold_fail_closed": True,
        "sandbox_command_render_plan_scaffold_deny_by_default": True,
        "sandbox_command_render_plan_requires_input_materialization_plan": True,
        "sandbox_command_render_plan_requires_workspace_preparation_preflight": True,
        "sandbox_command_render_plan_requires_workspace_plan": True,
        "sandbox_command_render_plan_requires_materialization_preflight": True,
        "sandbox_command_render_plan_requires_envelope_scaffold": True,
        "sandbox_command_render_plan_requires_request_preflight": True,
        "sandbox_command_render_plan_requires_policy_matrix": True,
        "sandbox_command_render_plan_requires_known_capability": True,
        "sandbox_command_render_plan_requires_known_policy": True,
        "sandbox_command_render_plan_requires_operator_authorization": True,
        "sandbox_command_render_plan_requires_approval_lineage": True,
        "sandbox_command_render_plan_requires_final_gate": True,
        "sandbox_command_render_plan_requires_dry_run_envelope": True,
        "sandbox_command_render_plan_requires_rollback_plan": True,
        "sandbox_command_render_plan_requires_post_execution_evidence": True,
        "sandbox_command_render_plan_rejects_unknown_capability": True,
        "sandbox_command_render_plan_rejects_unknown_policy": True,
        "sandbox_command_render_plan_rejects_orphans": True,
        "sandbox_command_render_plan_rejects_stale_records": True,
        "source_input_materialization_plan_scaffold_exists": True,
        "source_input_materialization_plan_scaffold_fail_closed": True,
        "source_input_materialization_plan_scaffold_deny_by_default": True,
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_workspace_plan_strategy": "explicit_plan_only",
        "sandbox_workspace_preparation_strategy": (
            "preflight_only_no_directory_creation"
        ...
        "production_paths_mutated": False,
        "production_secrets_accessed": False,
        "source_input_materialization_plan_generation_enabled": False,
        "source_input_materialization_plan_materialized": False,
        "source_input_materialization_plan_executable": False,
        "source_input_materialization_enabled": False,
        "source_inputs_materialized": False,
        "source_command_rendering_enabled": False,
        "source_sandbox_execution_enabled": False,
        "source_result_generation_enabled": False,
        "source_execution_performed": False,
...
def test_validate_retry_real_execution_sandbox_command_render_plan_scaffold_accepts_blocked_fail_closed() -> None:
    result = (
        validate_replay_lifecycle_retry_real_execution_sandbox_command_render_plan_scaffold(
            _real_execution_sandbox_command_render_plan_scaffold()
        )
    )
    ...
    assert result["severity"] == "info"
    assert result["reasons"] == []
    assert result["sandbox_command_render_plan_scaffold_status"] == "blocked"
    assert result["sandbox_command_render_plan_scaffold_fail_closed"] is True
    assert result["sandbox_command_render_plan_scaffold_deny_by_default"] is True
    assert result["sandbox_command_render_plan_generation_enabled"] is False
    assert result["sandbox_command_render_plan_materialized"] is False
    assert result["sandbox_command_render_plan_executable"] is False
    assert result["sandbox_command_rendering_enabled"] is False
    assert result["sandbox_command_rendered"] is False
    assert result["sandbox_rendered_command_validated"] is False
    assert result["sandbox_input_materialization_plan_generation_enabled"] is False
    assert result["sandbox_input_materialization_plan_executable"] is False
    assert result["sandbox_input_materialization_enabled"] is False
    assert result["sandbox_inputs_materialized"] is False
...
def test_validate_retry_real_execution_sandbox_command_render_plan_scaffold_rejects_plan_generation_enabled() -> None:
    record = _real_execution_sandbox_command_render_plan_scaffold(
        sandbox_command_render_plan_generation_enabled=True
    )
    record["payload"]["sandbox_command_render_plan_generation_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_command_render_plan_generation_enabled_must_be_false" in result[
        "reasons"
    ]
    assert (
        "payload_sandbox_command_render_plan_generation_enabled_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_command_render_plan_scaffold_rejects_command_rendering_enabled() -> None:
    record = _real_execution_sandbox_command_render_plan_scaffold(
        sandbox_command_rendering_enabled=True
    )
    record["payload"]["sandbox_command_rendering_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_command_rendering_enabled_must_be_false" in result["reasons"]
    assert "payload_sandbox_command_rendering_enabled_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_command_render_plan_scaffold_rejects_command_rendered() -> None:
    record = _real_execution_sandbox_command_render_plan_scaffold(
        sandbox_command_rendered=True
    )
    record["payload"]["sandbox_command_rendered"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_command_rendered_must_be_false" in result["reasons"]
    assert "payload_sandbox_command_rendered_must_be_false" in result["reasons"]


def test_validate_retry_real_execution_sandbox_command_render_plan_scaffold_rejects_rendered_command_validated() -> None:
    record = _real_execution_sandbox_command_render_plan_scaffold(
        sandbox_rendered_command_validated=True
    )
    record["payload"]["sandbox_rendered_command_validated"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_rendered_command_validated_must_be_false" in result["reasons"]
    assert "payload_sandbox_rendered_command_validated_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_command_render_plan_scaffold_rejects_sandbox_execution_enabled() -> None:
    record = _real_execution_sandbox_command_render_plan_scaffold(
        sandbox_execution_enabled=True
    )
    record["payload"]["sandbox_execution_enabled"] = True
...
def _real_execution_sandbox_rendered_command_scaffold(**overrides):
    item = {
        "type": (
        ...
        "proposal_id": "proposal-1",
        "rendered_command_id": "rendered-1",
        "schema_version": "real-execution-sandbox-rendered-command-scaffold/v1",
        ...
        "source_command_render_plan_scaffold_exists": True,
        "source_command_render_plan_scaffold_fail_closed": True,
        "source_command_render_plan_scaffold_deny_by_default": True,
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_workspace_plan_strategy": "explicit_plan_only",
        "sandbox_workspace_preparation_strategy": (
            "preflight_only_no_directory_creation"
        ...
        "production_paths_mutated": False,
        "production_secrets_accessed": False,
        "source_command_render_plan_generation_enabled": False,
        "source_command_render_plan_materialized": False,
        "source_command_render_plan_executable": False,
        "source_command_rendering_enabled": False,
        "source_command_rendered": False,
        "source_rendered_command_validated": False,
        "source_sandbox_execution_enabled": False,
        "source_result_generation_enabled": False,
        "source_execution_performed": False,
        "source_subprocess_invoked": False,
...
def test_validate_retry_real_execution_sandbox_rendered_command_scaffold_accepts_blocked_fail_closed() -> None:
    result = (
        validate_replay_lifecycle_retry_real_execution_sandbox_rendered_command_scaffold(
            _real_execution_sandbox_rendered_command_scaffold()
        )
    )
    ...
    assert result["severity"] == "info"
    assert result["reasons"] == []
    assert result["sandbox_rendered_command_scaffold_status"] == "blocked"
    assert result["sandbox_rendered_command_scaffold_fail_closed"] is True
    assert result["sandbox_rendered_command_scaffold_deny_by_default"] is True
    assert result["sandbox_rendered_command_generation_enabled"] is False
    assert result["sandbox_rendered_command_materialized"] is False
    assert result["sandbox_rendered_command_executable"] is False
    assert result["sandbox_rendered_command_validated"] is False
    assert result["sandbox_command_render_plan_generation_enabled"] is False
    assert result["sandbox_command_render_plan_executable"] is False
    assert result["sandbox_command_rendering_enabled"] is False
...
def test_validate_retry_real_execution_sandbox_rendered_command_scaffold_rejects_generation_enabled() -> None:
    record = _real_execution_sandbox_rendered_command_scaffold(
        sandbox_rendered_command_generation_enabled=True
    )
    record["payload"]["sandbox_rendered_command_generation_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_rendered_command_generation_enabled_must_be_false" in result[
        "reasons"
    ]
    assert (
        "payload_sandbox_rendered_command_generation_enabled_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_rendered_command_scaffold_rejects_materialized() -> None:
    record = _real_execution_sandbox_rendered_command_scaffold(
        sandbox_rendered_command_materialized=True
    )
    record["payload"]["sandbox_rendered_command_materialized"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_rendered_command_materialized_must_be_false" in result[
        "reasons"
    ]
    assert "payload_sandbox_rendered_command_materialized_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_rendered_command_scaffold_rejects_executable() -> None:
    record = _real_execution_sandbox_rendered_command_scaffold(
        sandbox_rendered_command_executable=True
    )
    record["payload"]["sandbox_rendered_command_executable"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_rendered_command_executable_must_be_false" in result[
        "reasons"
    ]
    assert "payload_sandbox_rendered_command_executable_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_rendered_command_scaffold_rejects_validated() -> None:
    record = _real_execution_sandbox_rendered_command_scaffold(
        sandbox_rendered_command_validated=True
    )
    record["payload"]["sandbox_rendered_command_validated"] = True
...
def test_validate_retry_real_execution_sandbox_rendered_command_scaffold_rejects_execution() -> None:
    record = _real_execution_sandbox_rendered_command_scaffold(
        sandbox_execution_enabled=True,
        execution_performed=True,
        subprocess_invoked=True,
    ...
    assert result["severity"] == "critical"
    assert "sandbox_execution_enabled_must_be_false" in result["reasons"]
    assert "execution_performed_must_be_false" in result["reasons"]
    assert "subprocess_invoked_must_be_false" in result["reasons"]


def _real_execution_sandbox_rendered_command_validation_scaffold(**overrides):
    item = {
        "type": (
        ...
        "real_execution_sandbox_rendered_command_validation_scaffold_id": (
            "sandbox-rendered-command-validation-1"
        ),
        "real_execution_sandbox_rendered_command_scaffold_id": (
            "sandbox-rendered-command-1"
        ),
        ...
        "source_rendered_command_scaffold_exists": True,
        "source_rendered_command_scaffold_fail_closed": True,
        "source_rendered_command_scaffold_deny_by_default": True,
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_workspace_plan_strategy": "explicit_plan_only",
        "sandbox_workspace_preparation_strategy": (
            "preflight_only_no_directory_creation"
        ...
        "production_paths_mutated": False,
        "production_secrets_accessed": False,
        "source_rendered_command_generation_enabled": False,
        "source_rendered_command_materialized": False,
        "source_rendered_command_executable": False,
        "source_rendered_command_validated": False,
        "source_sandbox_execution_enabled": False,
        "source_result_generation_enabled": False,
        "source_execution_performed": False,
...
def test_validate_retry_real_execution_sandbox_rendered_command_validation_scaffold_accepts_blocked_fail_closed() -> None:
    result = (
        validate_replay_lifecycle_retry_real_execution_sandbox_rendered_command_validation_scaffold(
            _real_execution_sandbox_rendered_command_validation_scaffold()
        )
    )
    ...
    assert result["severity"] == "info"
    assert result["reasons"] == []
    assert result["sandbox_rendered_command_validation_scaffold_status"] == "blocked"
    assert result["sandbox_rendered_command_validation_scaffold_fail_closed"] is True
    assert result["sandbox_rendered_command_validation_scaffold_deny_by_default"] is True
    assert result["sandbox_rendered_command_validation_enabled"] is False
    assert result["sandbox_rendered_command_validation_performed"] is False
    assert result["sandbox_rendered_command_validation_passed"] is False
    assert result["sandbox_rendered_command_validation_failed"] is False
    assert result["sandbox_rendered_command_generation_enabled"] is False
    assert result["sandbox_rendered_command_executable"] is False
    assert result["sandbox_rendered_command_validated"] is False
    assert result["sandbox_execution_enabled"] is False
...
def test_validate_retry_real_execution_sandbox_rendered_command_validation_scaffold_rejects_validation_enabled() -> None:
    record = _real_execution_sandbox_rendered_command_validation_scaffold(
        sandbox_rendered_command_validation_enabled=True
    )
    record["payload"]["sandbox_rendered_command_validation_enabled"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_rendered_command_validation_enabled_must_be_false" in result[
        "reasons"
    ]
    assert (
        "payload_sandbox_rendered_command_validation_enabled_must_be_false"
        in result["reasons"]
    )


def test_validate_retry_real_execution_sandbox_rendered_command_validation_scaffold_rejects_validation_performed() -> None:
    record = _real_execution_sandbox_rendered_command_validation_scaffold(
        sandbox_rendered_command_validation_performed=True
    )
    record["payload"]["sandbox_rendered_command_validation_performed"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_rendered_command_validation_performed_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_rendered_command_validation_scaffold_rejects_validation_passed() -> None:
    record = _real_execution_sandbox_rendered_command_validation_scaffold(
        sandbox_rendered_command_validation_passed=True
    )
    record["payload"]["sandbox_rendered_command_validation_passed"] = True
    ...
    assert result["valid"] is False
    assert result["severity"] == "critical"
    assert "sandbox_rendered_command_validation_passed_must_be_false" in result[
        "reasons"
    ]


def test_validate_retry_real_execution_sandbox_rendered_command_validation_scaffold_rejects_rendered_command_validated() -> None:
    record = _real_execution_sandbox_rendered_command_validation_scaffold(
        sandbox_rendered_command_validated=True
    )
    record["payload"]["sandbox_rendered_command_validated"] = True
...
def test_validate_retry_real_execution_sandbox_rendered_command_validation_scaffold_rejects_execution() -> None:
    record = _real_execution_sandbox_rendered_command_validation_scaffold(
        sandbox_execution_enabled=True,
        execution_performed=True,
        subprocess_invoked=True,
