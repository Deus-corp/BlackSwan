...
from tests.unit.swarms.test_build_real_execution_sandbox_materialization_preflight_scaffold import (
    _envelope_record,
)
...
def test_build_sandbox_workspace_plan_scaffold_record_is_fail_closed() -> None:
    record = build_real_execution_sandbox_workspace_plan_scaffold_record(
        _materialization_record()
    )

    assert record["type"] == SANDBOX_WORKSPACE_PLAN_SCAFFOLD_TYPE
    assert record["schema_version"] == SANDBOX_WORKSPACE_PLAN_SCAFFOLD_SCHEMA_VERSION
    assert record["sandbox_workspace_plan_scaffold_status"] == "blocked"
    assert (
        record["sandbox_workspace_plan_scaffold_kind"]
        == "fail_closed_sandbox_workspace_plan_scaffold"
    )
    assert record["sandbox_workspace_plan_scaffold_exists"] is True
    assert record["sandbox_workspace_plan_scaffold_fail_closed"] is True
    assert record["sandbox_workspace_plan_scaffold_deny_by_default"] is True
    assert record["sandbox_workspace_plan_requires_materialization_preflight"] is True
    assert record["sandbox_workspace_plan_requires_envelope_scaffold"] is True
    assert record["sandbox_request_allowed_input_paths"] == []
    assert record["sandbox_request_allowed_output_paths"] == []
    assert record["sandbox_workspace_plan_generation_allowed"] is False
    ...
    assert record["sandbox_workspace_directory_creation_allowed"] is False
    ...
    assert (
        record["reason"]
        == "sandbox_workspace_plan_scaffold_defined_blocked_not_runnable"
    )
    assert record["payload"]["real_execution_sandbox_workspace_plan_scaffold_id"] == (
        record["real_execution_sandbox_workspace_plan_scaffold_id"]
    )


def test_build_sandbox_workspace_plan_scaffold_record_is_stable() -> None:
    first = build_real_execution_sandbox_workspace_plan_scaffold_record(
        _materialization_record()
    )
    second = build_real_execution_sandbox_workspace_plan_scaffold_record(
        _materialization_record()
    )

    assert first["real_execution_sandbox_workspace_plan_scaffold_id"] == (
        second["real_execution_sandbox_workspace_plan_scaffold_id"]
    )


def test_build_sandbox_workspace_plan_scaffold_rejects_materialization_preflight_enabled() -> None:
    with pytest.raises(
        ValueError,
        match="sandbox_materialization_preflight_enabled=false",
    ):
        build_real_execution_sandbox_workspace_plan_scaffold_record(
            _materialization_record(
...
def test_build_sandbox_workspace_plan_scaffold_rejects_materialization_preflight_passed() -> None:
    with pytest.raises(
        ValueError,
        match="sandbox_materialization_preflight_passed=false",
    ):
        build_real_execution_sandbox_workspace_plan_scaffold_record(
            _materialization_record(
...
def test_build_sandbox_workspace_plan_scaffold_rejects_envelope_executable() -> None:
    with pytest.raises(
        ValueError,
        match="sandbox_request_envelope_executable=false",
    ):
        build_real_execution_sandbox_workspace_plan_scaffold_record(
            _materialization_record(sandbox_request_envelope_executable=True)
        )


def test_build_sandbox_workspace_plan_scaffold_rejects_workspace_creation_enabled() -> None:
    with pytest.raises(
        ValueError,
        match="sandbox_workspace_creation_enabled=false",
    ):
        build_real_execution_sandbox_workspace_plan_scaffold_record(
            _materialization_record(sandbox_workspace_creation_enabled=True)
        )


def test_build_sandbox_workspace_plan_scaffold_rejects_input_materialization_enabled() -> None:
    with pytest.raises(
        ValueError,
        match="sandbox_input_materialization_enabled=false",
    ):
        build_real_execution_sandbox_workspace_plan_scaffold_record(
            _materialization_record(sandbox_input_materialization_enabled=True)
        )


def test_build_sandbox_workspace_plan_scaffold_rejects_sandbox_execution_enabled() -> None:
    with pytest.raises(
        ValueError,
        match="sandbox_execution_enabled=false",
    ):
        build_real_execution_sandbox_workspace_plan_scaffold_record(
            _materialization_record(sandbox_execution_enabled=True)
        )