...
def test_global_brief_surfaces_sandbox_materialization_preflight_scaffold_observability() -> None:
    brief = build_global_swarm_brief(
        snapshot={"active_swarm_counts": {"overseer": 1, "security": 1}},
        security_validation={
            "security_validation_records": 1,
    ...
    assert "Sandbox materialization preflight scaffold observed" in text
    assert "blocked=1" in text
    assert "fail_closed=1" in text
    assert "deny_by_default=1" in text
    assert "linkage_complete=1" in text
...
def test_global_brief_surfaces_sandbox_workspace_plan_scaffold_observability() -> None:
    brief = build_global_swarm_brief(
        snapshot={"active_swarm_counts": {"overseer": 1, "security": 1}},
        security_validation={
            "security_validation_records": 1,
    ...
    assert "Sandbox workspace plan scaffold observed" in text
    assert "blocked=1" in text
    assert "fail_closed=1" in text
    assert "deny_by_default=1" in text
    assert "linkage_complete=1" in text
...
def test_global_brief_surfaces_sandbox_workspace_preparation_preflight_scaffold_observability() -> None:
    brief = build_global_swarm_brief(
        snapshot={"active_swarm_counts": {"overseer": 1, "security": 1}},
        security_validation={
            "security_validation_records": 1,
    ...
    assert "Sandbox workspace preparation preflight scaffold observed" in text
    assert "blocked=1" in text
    assert "fail_closed=1" in text
    assert "deny_by_default=1" in text
    assert "linkage_complete=1" in text
...
def test_global_brief_surfaces_sandbox_input_materialization_plan_scaffold_observability() -> None:
    brief = build_global_swarm_brief(
        snapshot={"active_swarm_counts": {"overseer": 1, "security": 1}},
        security_validation={
            "security_validation_records": 1,
    ...
    assert "Sandbox input materialization plan scaffold observed" in text
    assert "blocked=1" in text
    assert "fail_closed=1" in text
    assert "deny_by_default=1" in text
    assert "linkage_complete=1" in text
    ...
    assert "plan_executable=0" in text
    assert "input_materialization_enabled=0" in text
    assert "inputs_materialized=0" in text
    assert "command_rendering_enabled=0" in text
    assert "sandbox_execution_enabled=0" in text
    assert "execution_performed=0" in text
    assert "subprocess_invoked=0" in text
...
def test_global_brief_surfaces_sandbox_command_render_plan_scaffold_observability() -> None:
    brief = build_global_swarm_brief(
        snapshot={"active_swarm_counts": {"overseer": 1, "security": 1}},
        security_validation={
            "security_validation_records": 1,
    ...
    assert "Sandbox command render plan scaffold observed" in text
    assert "blocked=1" in text
    assert "fail_closed=1" in text
    assert "deny_by_default=1" in text
    assert "linkage_complete=1" in text
    ...
    assert "rendered_command_validated=0" in text
    assert "sandbox_execution_enabled=0" in text
    assert "execution_performed=0" in text
    assert "subprocess_invoked=0" in text
    assert "real_execution_enabled=0" in text
...
def test_global_brief_surfaces_sandbox_rendered_command_validation_scaffold_observability() -> None:
    brief = build_global_swarm_brief(
        snapshot={"active_swarm_counts": {"overseer": 1, "security": 1}},
        security_validation={
            "security_validation_records": 1,
    ...
    assert "Sandbox rendered command validation scaffold observed" in text
    assert "blocked=1" in text
    assert "fail_closed=1" in text
    assert "deny_by_default=1" in text
    assert "linkage_complete=1" in text
