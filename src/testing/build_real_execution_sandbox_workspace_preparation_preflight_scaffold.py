"""Build fail-closed sandbox workspace preparation preflight scaffold records.

This artifact prepares a blocked workspace preparation preflight scaffold from a
valid sandbox workspace plan scaffold.

It intentionally does not:
- generate workspace plans,
- materialize workspace plans,
...
REQUIRED_WORKSPACE_PLAN_TRUE_FLAGS = [
...
REQUIRED_WORKSPACE_PLAN_FALSE_FLAGS = [
...
EXPECTED_WORKSPACE_PLAN_STRINGS = {
    "schema_version": EXPECTED_SANDBOX_WORKSPACE_PLAN_SCAFFOLD_SCHEMA_VERSION,
...
def _require_bool(record: Mapping[str, Any], key: str, expected: bool) -> None:
    ...
    if value is not expected:
        ...
        raise ValueError(
            "source sandbox workspace plan scaffold requires "
            f"{key}={expected_text}"
        )


def _validate_sandbox_workspace_plan_scaffold(record: Mapping[str, Any]) -> None:
    ...
    for key, expected in EXPECTED_WORKSPACE_PLAN_STRINGS.items():
    ...
    for key in REQUIRED_WORKSPACE_PLAN_TRUE_FLAGS:
        _require_bool(record, key, True)

    for key in REQUIRED_WORKSPACE_PLAN_FALSE_FLAGS:
    ...
    if missing:
        raise ValueError(
            "source sandbox workspace plan scaffold missing required field(s): "
            + ",".join(missing)
        )

    if _safe_list(record.get("sandbox_request_allowed_input_paths")):
        raise ValueError(
            "source sandbox workspace plan scaffold must not allow input paths"
        )
    if _safe_list(record.get("sandbox_request_allowed_output_paths")):
        raise ValueError(
            "source sandbox workspace plan scaffold must not allow output paths"
        )


def build_real_execution_sandbox_workspace_preparation_preflight_scaffold_record(
    sandbox_workspace_plan_scaffold: Mapping[str, Any],
    *,
    source: str = "real-execution-sandbox-workspace-preparation-preflight-scaffold",
) -> dict[str, Any]:
    """Build blocked sandbox workspace preparation preflight scaffold record."""
    _validate_sandbox_workspace_plan_scaffold(sandbox_workspace_plan_scaffold)

    workspace_plan_id = _clean(
        sandbox_workspace_plan_scaffold.get(
    ...
    contract_id = _clean(
        sandbox_workspace_plan_scaffold.get("real_execution_adapter_contract_id")
    )
    rendered_command_id = _clean(
        sandbox_workspace_plan_scaffold.get("rendered_command_id")
    )
    proposal_id = _clean(sandbox_workspace_plan_scaffold.get("proposal_id"))

    workspace_preparation_preflight_id = _stable_id(
        "replay-retry-real-execution-sandbox-workspace-preparation-preflight-scaffold",
        workspace_plan_id,
        materialization_id,
        envelope_id,
        preflight_id,
    ...
    payload = {
        ...
        "post_repair_evidence_check_id": _clean(
            sandbox_workspace_plan_scaffold.get("post_repair_evidence_check_id")
        ),
        "guarded_repair_execution_result_id": _clean(
            sandbox_workspace_plan_scaffold.get("guarded_repair_execution_result_id")
        ),
        "controlled_execution_result_id": _clean(
            sandbox_workspace_plan_scaffold.get("controlled_execution_result_id")
        ),
        "rendered_command_id": rendered_command_id,
        "plan_id": _clean(sandbox_workspace_plan_scaffold.get("plan_id")),
        "proposal_id": proposal_id,
        "approval_id": _clean(sandbox_workspace_plan_scaffold.get("approval_id")),
        "decision_mode": _clean(sandbox_workspace_plan_scaffold.get("decision_mode"))
        or "manual",
        "timeout_profile": _clean(
            sandbox_workspace_plan_scaffold.get("timeout_profile")
        )
        or "standard",
        "schema_version": (
            SANDBOX_WORKSPACE_PREPARATION_PREFLIGHT_SCAFFOLD_SCHEMA_VERSION
        ...
        "source_workspace_plan_scaffold_schema_version": _clean(
            sandbox_workspace_plan_scaffold.get("schema_version")
        ),
        ...
        "sandbox_request_required_fields": list(
            _safe_list(
                sandbox_workspace_plan_scaffold.get("sandbox_request_required_fields")
            )
        ),
        ...
        "sandbox_production_write_policy": "deny",
        "sandbox_external_side_effect_policy": "deny",
        "source_workspace_plan_scaffold_status": _clean(
            sandbox_workspace_plan_scaffold.get(
                "sandbox_workspace_plan_scaffold_status"
            )
        ),
        "source_workspace_plan_scaffold_exists": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_workspace_plan_scaffold_exists")
        ),
        "source_workspace_plan_scaffold_fail_closed": bool(
            sandbox_workspace_plan_scaffold.get(
                "sandbox_workspace_plan_scaffold_fail_closed"
            )
        ),
        "source_workspace_plan_scaffold_deny_by_default": bool(
            sandbox_workspace_plan_scaffold.get(
                "sandbox_workspace_plan_scaffold_deny_by_default"
            )
        ),
        "source_workspace_plan_generation_enabled": bool(
            sandbox_workspace_plan_scaffold.get(
                "sandbox_workspace_plan_generation_enabled"
            )
        ),
        "source_workspace_plan_materialized": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_workspace_plan_materialized")
        ),
        "source_workspace_plan_executable": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_workspace_plan_executable")
        ),
        ...
        "source_workspace_created": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_workspace_created")
        ),
        "source_cleanup_registered": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_workspace_cleanup_registered")
        ),
        ...
        "source_workspace_creation_enabled": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_workspace_creation_enabled")
        ),
        "source_input_materialization_enabled": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_input_materialization_enabled")
        ),
        "source_command_rendering_enabled": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_command_rendering_enabled")
        ),
        "source_sandbox_execution_enabled": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_execution_enabled")
        ),
        "source_result_generation_enabled": bool(
            sandbox_workspace_plan_scaffold.get("sandbox_result_generation_enabled")
        ),
        "source_execution_performed": bool(
            sandbox_workspace_plan_scaffold.get("execution_performed")
        ),
        "source_subprocess_invoked": bool(
            sandbox_workspace_plan_scaffold.get("subprocess_invoked")
        ),
        "source_real_execution_enabled": bool(
            sandbox_workspace_plan_scaffold.get("real_execution_enabled")
        ),
        "source_external_side_effects_performed": bool(
            sandbox_workspace_plan_scaffold.get("external_side_effects_performed")
        ),
        "source_production_paths_mutated": bool(
            sandbox_workspace_plan_scaffold.get("production_paths_mutated")
        ),
        "source_production_secrets_accessed": bool(
            sandbox_workspace_plan_scaffold.get("production_secrets_accessed")
        ),
        "recommended_next_action": (
    ...
    return {
        "type": SANDBOX_WORKSPACE_PREPARATION_PREFLIGHT_SCAFFOLD_TYPE,
        **payload,
        "source": source,
        "payload": payload,
    }
...
def _matches_filters(
    ...
    *,
    ...
    if (
        workspace_plan_id
        and _clean(record.get("real_execution_sandbox_workspace_plan_scaffold_id"))
        != workspace_plan_id
    ):
        return False
    return True
...
def _find_existing_workspace_preparation_preflight(
    ...
    *,
    ...
    for item in records:
        if item.get("type") != SANDBOX_WORKSPACE_PREPARATION_PREFLIGHT_SCAFFOLD_TYPE:
            continue
        if (
            _clean(item.get("real_execution_sandbox_workspace_plan_scaffold_id"))
            == workspace_plan_id
        ):
            return item
    return None
...
async def build_real_execution_sandbox_workspace_preparation_preflight_scaffold_records(
    args: argparse.Namespace,
) -> list[dict[str, Any]]:
    db_path = str(args.db_path or config.crdt_db_path)
    source = (
        _clean(getattr(args, "source", ""))
        or "real-execution-sandbox-workspace-preparation-preflight-scaffold"
    )
    proposal_id = _clean(getattr(args, "proposal_id", ""))
    rendered_command_id = _clean(getattr(args, "rendered_command_id", ""))
    workspace_plan_id = _clean(
        getattr(args, "real_execution_sandbox_workspace_plan_scaffold_id", "")
    )
    ...
    for workspace_plan in workspace_plans:
        current_workspace_plan_id = _clean(
            workspace_plan.get("real_execution_sandbox_workspace_plan_scaffold_id")
        )
        if _find_existing_workspace_preparation_preflight(
            records,
            workspace_plan_id=current_workspace_plan_id,
        ):
            logger.info(
                "Skipping duplicate sandbox workspace preparation preflight scaffold: "
                "workspace_plan_id=%s",
                current_workspace_plan_id,
            )
            continue
        ...
        logger.info(
            "Published sandbox workspace preparation preflight scaffold: "
            "preparation_preflight_id=%s status=%s preflight_enabled=%s "
            "directory_creation_enabled=%s workspace_created=%s "
            "sandbox_execution_enabled=%s",
            record.get(
                "real_execution_sandbox_workspace_preparation_preflight_scaffold_id"
            ),
            record.get("sandbox_workspace_preparation_preflight_scaffold_status"),
            record.get("sandbox_workspace_preparation_preflight_enabled"),
            record.get("sandbox_workspace_directory_creation_enabled"),
            record.get("sandbox_workspace_created"),
            record.get("sandbox_execution_enabled"),
        )

    logger.info(
        "Sandbox workspace preparation preflight scaffold builder completed: "
        "preparation_preflights=%s",
        len(results),
    )
    return results
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build blocked sandbox workspace preparation preflight scaffold records."
        ),
    )
    ...
    parser.add_argument(
        "--real-execution-sandbox-workspace-plan-scaffold-id",
        default="",
    )
    parser.add_argument(
        "--source",
        default="real-execution-sandbox-workspace-preparation-preflight-scaffold",
    )
...
async def _async_main() -> None:
    ...
    results = (
        await build_real_execution_sandbox_workspace_preparation_preflight_scaffold_records(
            args
        )
    )
    ...
    if args.json:
        ...
        print(
            "Sandbox workspace preparation preflight scaffold builder completed: "
            f"preparation_preflights={len(results)}"
        )
