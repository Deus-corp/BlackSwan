...
def inspect_retry_governance_trail_from_records(
    ...
    *,
    ...
    real_execution_sandbox_command_render_plan_scaffolds = [
        item
        for item in trail_records
        if item.get("type")
        == "replay_lifecycle_retry_real_execution_sandbox_command_render_plan_scaffold"
    ]
    real_execution_sandbox_rendered_command_scaffolds = [
        item
        for item in trail_records
        if item.get("type")
        == "replay_lifecycle_retry_real_execution_sandbox_rendered_command_scaffold"
    ]
    real_execution_sandbox_rendered_command_validation_scaffolds = [
        item
        for item in trail_records
        if item.get("type")
        == "replay_lifecycle_retry_real_execution_sandbox_rendered_command_validation_scaffold"
    ...
    real_execution_sandbox_workspace_preparation_preflight_scaffold_statuses = Counter(
        str(
            item.get("sandbox_workspace_preparation_preflight_scaffold_status")
            or "unknown"
        ).strip()
        or "unknown"
        for item in real_execution_sandbox_workspace_preparation_preflight_scaffolds
    )
    real_execution_sandbox_workspace_preparation_preflight_scaffold_schema_versions = Counter(
        str(item.get("schema_version") or "unknown").strip() or "unknown"
        for item in real_execution_sandbox_workspace_preparation_preflight_scaffolds
    )
    real_execution_sandbox_workspace_preparation_preflight_scaffold_next_actions = Counter(
        str(item.get("recommended_next_action") or "unknown").strip()
        or "unknown"
        for item in real_execution_sandbox_workspace_preparation_preflight_scaffolds
    )

    def _sandbox_workspace_preparation_preflight_scaffold_bool_counter(
        key: str,
    ) -> Counter[str]:
        return Counter(
            str(bool(item.get(key))).lower()
    ...
    real_execution_sandbox_workspace_preparation_preflight_scaffold_fail_closed = (
        _sandbox_workspace_preparation_preflight_scaffold_bool_counter(
    ...
    real_execution_sandbox_command_render_plan_scaffold_statuses = Counter(
        str(item.get("sandbox_command_render_plan_scaffold_status") or "unknown").strip()
        or "unknown"
        for item in real_execution_sandbox_command_render_plan_scaffolds
    )
    real_execution_sandbox_command_render_plan_scaffold_schema_versions = Counter(
        str(item.get("schema_version") or "unknown").strip() or "unknown"
        for item in real_execution_sandbox_command_render_plan_scaffolds
    )
    real_execution_sandbox_command_render_plan_scaffold_next_actions = Counter(
        str(item.get("recommended_next_action") or "unknown").strip()
        or "unknown"
        for item in real_execution_sandbox_command_render_plan_scaffolds
    )

    def _sandbox_command_render_plan_scaffold_bool_counter(
        key: str,
    ) -> Counter[str]:
        return Counter(
            str(bool(item.get(key))).lower()
    ...
    real_execution_sandbox_command_render_plan_scaffold_fail_closed = (
        _sandbox_command_render_plan_scaffold_bool_counter(
    ...
    real_execution_sandbox_rendered_command_scaffold_statuses = Counter(
        str(item.get("sandbox_rendered_command_scaffold_status") or "unknown").strip()
        or "unknown"
        for item in real_execution_sandbox_rendered_command_scaffolds
    )
    real_execution_sandbox_rendered_command_scaffold_schema_versions = Counter(
        str(item.get("schema_version") or "unknown").strip() or "unknown"
        for item in real_execution_sandbox_rendered_command_scaffolds
    )
    real_execution_sandbox_rendered_command_scaffold_next_actions = Counter(
        str(item.get("recommended_next_action") or "unknown").strip()
        or "unknown"
        for item in real_execution_sandbox_rendered_command_scaffolds
    )

    def _sandbox_rendered_command_scaffold_bool_counter(key: str) -> Counter[str]:
        return Counter(
            str(bool(item.get(key))).lower()
            for item in real_execution_sandbox_rendered_command_scaffolds
        )
    ...
    real_execution_sandbox_rendered_command_scaffold_fail_closed = (
        _sandbox_rendered_command_scaffold_bool_counter(
    ...
    real_execution_sandbox_rendered_command_scaffold_input_plan_generation_enabled = (
        _sandbox_rendered_command_scaffold_bool_counter(
            "sandbox_input_materialization_plan_generation_enabled"
        )
    )
    real_execution_sandbox_rendered_command_scaffold_input_plan_materialized = (
        _sandbox_rendered_command_scaffold_bool_counter(
            "sandbox_input_materialization_plan_materialized"
        )
    )
    real_execution_sandbox_rendered_command_scaffold_input_plan_executable = (
        _sandbox_rendered_command_scaffold_bool_counter(
            "sandbox_input_materialization_plan_executable"
        )
    )
    real_execution_sandbox_rendered_command_scaffold_input_materialization_enabled = (
        _sandbox_rendered_command_scaffold_bool_counter(
            "sandbox_input_materialization_enabled"
        )
    )
    real_execution_sandbox_rendered_command_scaffold_inputs_materialized = (
        _sandbox_rendered_command_scaffold_bool_counter(
            "sandbox_inputs_materialized"
        )
    )
    real_execution_sandbox_rendered_command_scaffold_workspace_created = (
        _sandbox_rendered_command_scaffold_bool_counter("sandbox_workspace_created")
    )
    real_execution_sandbox_rendered_command_scaffold_cleanup_registered = (
        _sandbox_rendered_command_scaffold_bool_counter(
            "sandbox_workspace_cleanup_registered"
        )
    )
    real_execution_sandbox_rendered_command_scaffold_sandbox_execution_enabled = (
        _sandbox_rendered_command_scaffold_bool_counter("sandbox_execution_enabled")
    )
    ...
    real_execution_sandbox_rendered_command_scaffold_execution_performed = (
        _sandbox_rendered_command_scaffold_bool_counter("execution_performed")
    )
    real_execution_sandbox_rendered_command_scaffold_subprocess_invoked = (
        _sandbox_rendered_command_scaffold_bool_counter("subprocess_invoked")
    )
    real_execution_sandbox_rendered_command_scaffold_real_execution_enabled = (
        _sandbox_rendered_command_scaffold_bool_counter("real_execution_enabled")
    )
    ...
    real_execution_sandbox_rendered_command_scaffold_production_paths_mutated = (
        _sandbox_rendered_command_scaffold_bool_counter("production_paths_mutated")
    )
    real_execution_sandbox_rendered_command_scaffold_production_secrets_accessed = (
        _sandbox_rendered_command_scaffold_bool_counter("production_secrets_accessed")
    )
    real_execution_sandbox_rendered_command_validation_scaffold_statuses = Counter(
        str(
            item.get("sandbox_rendered_command_validation_scaffold_status")
            or "unknown"
        ).strip()
        or "unknown"
        for item in real_execution_sandbox_rendered_command_validation_scaffolds
    )
    real_execution_sandbox_rendered_command_validation_scaffold_schema_versions = Counter(
        str(item.get("schema_version") or "unknown").strip() or "unknown"
        for item in real_execution_sandbox_rendered_command_validation_scaffolds
    )
    real_execution_sandbox_rendered_command_validation_scaffold_next_actions = Counter(
        str(item.get("recommended_next_action") or "unknown").strip()
        or "unknown"
        for item in real_execution_sandbox_rendered_command_validation_scaffolds
    )

    def _sandbox_rendered_command_validation_scaffold_bool_counter(
        key: str,
    ) -> Counter[str]:
        return Counter(
            str(bool(item.get(key))).lower()
    ...
    real_execution_sandbox_rendered_command_validation_scaffold_fail_closed = (
        _sandbox_rendered_command_validation_scaffold_bool_counter(
    ...
    real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage = (
        _real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage_summary(
            real_execution_sandbox_workspace_plan_scaffolds=(
                real_execution_sandbox_workspace_plan_scaffolds
            ),
            real_execution_sandbox_workspace_preparation_preflight_scaffolds=(
    ...
    real_execution_sandbox_command_render_plan_scaffold_linkage = (
        _real_execution_sandbox_command_render_plan_scaffold_linkage_summary(
            real_execution_sandbox_input_materialization_plan_scaffolds=(
                real_execution_sandbox_input_materialization_plan_scaffolds
            ),
            real_execution_sandbox_command_render_plan_scaffolds=(
    ...
    real_execution_sandbox_rendered_command_scaffold_linkage = (
        _real_execution_sandbox_rendered_command_scaffold_linkage_summary(
            real_execution_sandbox_command_render_plan_scaffolds=(
                real_execution_sandbox_command_render_plan_scaffolds
            ),
            real_execution_sandbox_rendered_command_scaffolds=(
    ...
    real_execution_sandbox_rendered_command_validation_scaffold_linkage = (
        _real_execution_sandbox_rendered_command_validation_scaffold_linkage_summary(
            real_execution_sandbox_rendered_command_scaffolds=(
                real_execution_sandbox_rendered_command_scaffolds
            ),
            real_execution_sandbox_rendered_command_validation_scaffolds=(
    ...
    return {
        ...
        "counts": {
            ...
            "real_execution_sandbox_command_render_plan_scaffolds": len(
                real_execution_sandbox_command_render_plan_scaffolds
            ),
            "real_execution_sandbox_rendered_command_scaffolds": len(
                real_execution_sandbox_rendered_command_scaffolds
            ),
            "real_execution_sandbox_rendered_command_validation_scaffolds": len(
                real_execution_sandbox_rendered_command_validation_scaffolds
            ),
            "results": len(results),
        },
        ...
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_statuses": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_statuses
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_schema_versions": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_schema_versions
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_next_actions": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_next_actions
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_fail_closed": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_fail_closed
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_deny_by_default": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_deny_by_default
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_preflight_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_preflight_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_preflight_passed": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_preflight_passed
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_plan_generation_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_plan_generation_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_plan_materialized": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_plan_materialized
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_plan_executable": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_plan_executable
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_directory_creation_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_directory_creation_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_workspace_created": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_workspace_created
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_cleanup_registered": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_cleanup_registered
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_workspace_creation_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_workspace_creation_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_input_materialization_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_input_materialization_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_command_rendering_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_command_rendering_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_sandbox_execution_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_sandbox_execution_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_result_generation_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_result_generation_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_execution_performed": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_execution_performed
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_subprocess_invoked": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_subprocess_invoked
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_real_execution_enabled": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_real_execution_enabled
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_external_side_effects_performed": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_external_side_effects_performed
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_production_paths_mutated": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_production_paths_mutated
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_production_secrets_accessed": dict(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_production_secrets_accessed
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage": (
            real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage
        ...
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage_complete": bool(
            real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage.get(
                "real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage_complete"
            )
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_workspace_plan_matches": (
            real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage.get(
                "real_execution_sandbox_workspace_preparation_preflight_scaffold_workspace_plan_matches",
                0,
            )
        ...
        "real_execution_sandbox_command_render_plan_scaffold_statuses": dict(
            real_execution_sandbox_command_render_plan_scaffold_statuses
        ),
        "real_execution_sandbox_command_render_plan_scaffold_schema_versions": dict(
            real_execution_sandbox_command_render_plan_scaffold_schema_versions
        ),
        "real_execution_sandbox_command_render_plan_scaffold_next_actions": dict(
            real_execution_sandbox_command_render_plan_scaffold_next_actions
        ),
        "real_execution_sandbox_command_render_plan_scaffold_fail_closed": dict(
            real_execution_sandbox_command_render_plan_scaffold_fail_closed
        ),
        "real_execution_sandbox_command_render_plan_scaffold_deny_by_default": dict(
            real_execution_sandbox_command_render_plan_scaffold_deny_by_default
        ),
        "real_execution_sandbox_command_render_plan_scaffold_plan_generation_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_plan_generation_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_plan_materialized": dict(
            real_execution_sandbox_command_render_plan_scaffold_plan_materialized
        ),
        "real_execution_sandbox_command_render_plan_scaffold_plan_executable": dict(
            real_execution_sandbox_command_render_plan_scaffold_plan_executable
        ),
        "real_execution_sandbox_command_render_plan_scaffold_command_rendering_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_command_rendering_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_command_rendered": dict(
            real_execution_sandbox_command_render_plan_scaffold_command_rendered
        ),
        "real_execution_sandbox_command_render_plan_scaffold_rendered_command_validated": dict(
            real_execution_sandbox_command_render_plan_scaffold_rendered_command_validated
        ),
        "real_execution_sandbox_command_render_plan_scaffold_input_plan_generation_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_input_plan_generation_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_input_plan_materialized": dict(
            real_execution_sandbox_command_render_plan_scaffold_input_plan_materialized
        ),
        "real_execution_sandbox_command_render_plan_scaffold_input_plan_executable": dict(
            real_execution_sandbox_command_render_plan_scaffold_input_plan_executable
        ),
        "real_execution_sandbox_command_render_plan_scaffold_input_materialization_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_input_materialization_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_inputs_materialized": dict(
            real_execution_sandbox_command_render_plan_scaffold_inputs_materialized
        ),
        "real_execution_sandbox_command_render_plan_scaffold_preparation_preflight_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_preparation_preflight_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_preparation_preflight_passed": dict(
            real_execution_sandbox_command_render_plan_scaffold_preparation_preflight_passed
        ),
        "real_execution_sandbox_command_render_plan_scaffold_directory_creation_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_directory_creation_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_workspace_created": dict(
            real_execution_sandbox_command_render_plan_scaffold_workspace_created
        ),
        "real_execution_sandbox_command_render_plan_scaffold_cleanup_registered": dict(
            real_execution_sandbox_command_render_plan_scaffold_cleanup_registered
        ),
        "real_execution_sandbox_command_render_plan_scaffold_workspace_creation_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_workspace_creation_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_sandbox_execution_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_sandbox_execution_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_result_generation_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_result_generation_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_execution_performed": dict(
            real_execution_sandbox_command_render_plan_scaffold_execution_performed
        ),
        "real_execution_sandbox_command_render_plan_scaffold_subprocess_invoked": dict(
            real_execution_sandbox_command_render_plan_scaffold_subprocess_invoked
        ),
        "real_execution_sandbox_command_render_plan_scaffold_real_execution_enabled": dict(
            real_execution_sandbox_command_render_plan_scaffold_real_execution_enabled
        ),
        "real_execution_sandbox_command_render_plan_scaffold_external_side_effects_performed": dict(
            real_execution_sandbox_command_render_plan_scaffold_external_side_effects_performed
        ),
        "real_execution_sandbox_command_render_plan_scaffold_production_paths_mutated": dict(
            real_execution_sandbox_command_render_plan_scaffold_production_paths_mutated
        ),
        "real_execution_sandbox_command_render_plan_scaffold_production_secrets_accessed": dict(
            real_execution_sandbox_command_render_plan_scaffold_production_secrets_accessed
        ),
        "real_execution_sandbox_command_render_plan_scaffold_linkage": (
            real_execution_sandbox_command_render_plan_scaffold_linkage
        ...
        "real_execution_sandbox_command_render_plan_scaffold_linkage_complete": bool(
            real_execution_sandbox_command_render_plan_scaffold_linkage.get(
                "real_execution_sandbox_command_render_plan_scaffold_linkage_complete"
            )
        ),
        "real_execution_sandbox_command_render_plan_scaffold_input_plan_matches": (
            real_execution_sandbox_command_render_plan_scaffold_linkage.get(
                "real_execution_sandbox_command_render_plan_scaffold_input_plan_matches",
                0,
            )
        ...
        "real_execution_sandbox_rendered_command_scaffold_statuses": dict(
            real_execution_sandbox_rendered_command_scaffold_statuses
        ),
        "real_execution_sandbox_rendered_command_scaffold_schema_versions": dict(
            real_execution_sandbox_rendered_command_scaffold_schema_versions
        ),
        "real_execution_sandbox_rendered_command_scaffold_next_actions": dict(
            real_execution_sandbox_rendered_command_scaffold_next_actions
        ),
        "real_execution_sandbox_rendered_command_scaffold_fail_closed": dict(
            real_execution_sandbox_rendered_command_scaffold_fail_closed
        ),
        "real_execution_sandbox_rendered_command_scaffold_deny_by_default": dict(
            real_execution_sandbox_rendered_command_scaffold_deny_by_default
        ),
        "real_execution_sandbox_rendered_command_scaffold_generation_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_generation_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_materialized": dict(
            real_execution_sandbox_rendered_command_scaffold_materialized
        ),
        "real_execution_sandbox_rendered_command_scaffold_executable": dict(
            real_execution_sandbox_rendered_command_scaffold_executable
        ),
        "real_execution_sandbox_rendered_command_scaffold_validated": dict(
            real_execution_sandbox_rendered_command_scaffold_validated
        ),
        "real_execution_sandbox_rendered_command_scaffold_command_plan_generation_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_command_plan_generation_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_command_plan_materialized": dict(
            real_execution_sandbox_rendered_command_scaffold_command_plan_materialized
        ),
        "real_execution_sandbox_rendered_command_scaffold_command_plan_executable": dict(
            real_execution_sandbox_rendered_command_scaffold_command_plan_executable
        ),
        "real_execution_sandbox_rendered_command_scaffold_command_rendering_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_command_rendering_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_command_rendered": dict(
            real_execution_sandbox_rendered_command_scaffold_command_rendered
        ),
        "real_execution_sandbox_rendered_command_scaffold_input_plan_generation_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_input_plan_generation_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_input_plan_materialized": dict(
            real_execution_sandbox_rendered_command_scaffold_input_plan_materialized
        ),
        "real_execution_sandbox_rendered_command_scaffold_input_plan_executable": dict(
            real_execution_sandbox_rendered_command_scaffold_input_plan_executable
        ),
        "real_execution_sandbox_rendered_command_scaffold_input_materialization_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_input_materialization_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_inputs_materialized": dict(
            real_execution_sandbox_rendered_command_scaffold_inputs_materialized
        ),
        "real_execution_sandbox_rendered_command_scaffold_workspace_created": dict(
            real_execution_sandbox_rendered_command_scaffold_workspace_created
        ),
        "real_execution_sandbox_rendered_command_scaffold_cleanup_registered": dict(
            real_execution_sandbox_rendered_command_scaffold_cleanup_registered
        ),
        "real_execution_sandbox_rendered_command_scaffold_sandbox_execution_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_sandbox_execution_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_result_generation_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_result_generation_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_execution_performed": dict(
            real_execution_sandbox_rendered_command_scaffold_execution_performed
        ),
        "real_execution_sandbox_rendered_command_scaffold_subprocess_invoked": dict(
            real_execution_sandbox_rendered_command_scaffold_subprocess_invoked
        ),
        "real_execution_sandbox_rendered_command_scaffold_real_execution_enabled": dict(
            real_execution_sandbox_rendered_command_scaffold_real_execution_enabled
        ),
        "real_execution_sandbox_rendered_command_scaffold_external_side_effects_performed": dict(
            real_execution_sandbox_rendered_command_scaffold_external_side_effects_performed
        ),
        "real_execution_sandbox_rendered_command_scaffold_production_paths_mutated": dict(
            real_execution_sandbox_rendered_command_scaffold_production_paths_mutated
        ),
        "real_execution_sandbox_rendered_command_scaffold_production_secrets_accessed": dict(
            real_execution_sandbox_rendered_command_scaffold_production_secrets_accessed
        ),
        "real_execution_sandbox_rendered_command_scaffold_linkage": (
            real_execution_sandbox_rendered_command_scaffold_linkage
        ...
        "real_execution_sandbox_rendered_command_scaffold_linkage_complete": bool(
            real_execution_sandbox_rendered_command_scaffold_linkage.get(
                "real_execution_sandbox_rendered_command_scaffold_linkage_complete"
            )
        ),
        "real_execution_sandbox_rendered_command_scaffold_command_render_plan_matches": (
            real_execution_sandbox_rendered_command_scaffold_linkage.get(
                "real_execution_sandbox_rendered_command_scaffold_command_render_plan_matches",
                0,
            )
        ...
        "real_execution_sandbox_rendered_command_validation_scaffold_statuses": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_statuses
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_schema_versions": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_schema_versions
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_next_actions": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_next_actions
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_fail_closed": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_fail_closed
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_deny_by_default": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_deny_by_default
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_validation_enabled": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_validation_enabled
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_validation_performed": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_validation_performed
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_validation_passed": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_validation_passed
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_validation_failed": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_validation_failed
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_generation_enabled": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_generation_enabled
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_materialized": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_materialized
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_executable": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_executable
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_validated": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_validated
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_sandbox_execution_enabled": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_sandbox_execution_enabled
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_result_generation_enabled": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_result_generation_enabled
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_execution_performed": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_execution_performed
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_subprocess_invoked": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_subprocess_invoked
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_real_execution_enabled": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_real_execution_enabled
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_external_side_effects_performed": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_external_side_effects_performed
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_production_paths_mutated": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_production_paths_mutated
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_production_secrets_accessed": dict(
            real_execution_sandbox_rendered_command_validation_scaffold_production_secrets_accessed
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_linkage": (
            real_execution_sandbox_rendered_command_validation_scaffold_linkage
        ...
        "real_execution_sandbox_rendered_command_validation_scaffold_linkage_complete": bool(
            real_execution_sandbox_rendered_command_validation_scaffold_linkage.get(
                "real_execution_sandbox_rendered_command_validation_scaffold_linkage_complete"
            )
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_rendered_command_matches": (
            real_execution_sandbox_rendered_command_validation_scaffold_linkage.get(
                "real_execution_sandbox_rendered_command_validation_scaffold_rendered_command_matches",
                0,
            )
...
def _build_chain_ids(
    *,
    ...
    real_execution_sandbox_command_render_plan_scaffolds: list[Mapping[str, Any]],
    real_execution_sandbox_rendered_command_scaffolds: list[Mapping[str, Any]],
    real_execution_sandbox_rendered_command_validation_scaffolds: list[
        Mapping[str, Any]
    ],
    results: list[Mapping[str, Any]],
) -> dict[str, list[str]]:
    ...
    return {
        ...
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_ids": sorted(
            {
                str(
                    item.get(
        ...
        "real_execution_sandbox_command_render_plan_scaffold_ids": sorted(
            {
                str(
                    item.get(
        ...
        "real_execution_sandbox_rendered_command_scaffold_ids": sorted(
            {
                str(
                    item.get("real_execution_sandbox_rendered_command_scaffold_id")
                    or ""
                ).strip()
                for item in real_execution_sandbox_rendered_command_scaffolds
                if str(
        ...
        "real_execution_sandbox_rendered_command_validation_scaffold_ids": sorted(
            {
                str(
                    item.get(
...
def _real_execution_sandbox_workspace_preparation_preflight_scaffold_linkage_summary(
    *,
    real_execution_sandbox_workspace_plan_scaffolds: list[Mapping[str, Any]],
    real_execution_sandbox_workspace_preparation_preflight_scaffolds: list[
        Mapping[str, Any]
    ...
    workspace_plan_ids = {
        clean(item.get("real_execution_sandbox_workspace_plan_scaffold_id"))
        for item in real_execution_sandbox_workspace_plan_scaffolds
        if clean(item.get("real_execution_sandbox_workspace_plan_scaffold_id"))
    }

    preparation_workspace_plan_matches = 0
    preparation_orphans = 0

    for preparation in real_execution_sandbox_workspace_preparation_preflight_scaffolds:
        workspace_plan_id = clean(
            preparation.get("real_execution_sandbox_workspace_plan_scaffold_id")
        )
        if workspace_plan_id and workspace_plan_id in workspace_plan_ids:
            preparation_workspace_plan_matches += 1
        else:
            preparation_orphans += 1

    return {
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_workspace_plan_matches": (
            preparation_workspace_plan_matches
        ),
        "real_execution_sandbox_workspace_preparation_preflight_scaffold_orphans": (
            preparation_orphans
        ),
...
def _real_execution_sandbox_command_render_plan_scaffold_linkage_summary(
    *,
    real_execution_sandbox_input_materialization_plan_scaffolds: list[
        Mapping[str, Any]
    ],
    ...
    input_plan_ids = {
        clean(item.get("real_execution_sandbox_input_materialization_plan_scaffold_id"))
        for item in real_execution_sandbox_input_materialization_plan_scaffolds
        if clean(
            item.get("real_execution_sandbox_input_materialization_plan_scaffold_id")
        )
    }

    command_render_input_plan_matches = 0
    command_render_orphans = 0

    for command_render_plan in real_execution_sandbox_command_render_plan_scaffolds:
        input_plan_id = clean(
            command_render_plan.get(
                "real_execution_sandbox_input_materialization_plan_scaffold_id"
            )
        )
        if input_plan_id and input_plan_id in input_plan_ids:
            command_render_input_plan_matches += 1
        else:
            command_render_orphans += 1

    return {
        "real_execution_sandbox_command_render_plan_scaffold_input_plan_matches": (
            command_render_input_plan_matches
        ),
        "real_execution_sandbox_command_render_plan_scaffold_orphans": (
            command_render_orphans
        ),
        "real_execution_sandbox_command_render_plan_scaffold_linkage_complete": bool(
            real_execution_sandbox_command_render_plan_scaffolds
        )
        and command_render_orphans == 0,
    }


def _real_execution_sandbox_rendered_command_scaffold_linkage_summary(
    *,
    real_execution_sandbox_command_render_plan_scaffolds: list[Mapping[str, Any]],
    real_execution_sandbox_rendered_command_scaffolds: list[Mapping[str, Any]],
) -> dict[str, Any]:
    ...
    command_render_plan_ids = {
        clean(item.get("real_execution_sandbox_command_render_plan_scaffold_id"))
        for item in real_execution_sandbox_command_render_plan_scaffolds
        if clean(item.get("real_execution_sandbox_command_render_plan_scaffold_id"))
    }

    rendered_command_plan_matches = 0
    rendered_command_orphans = 0

    for rendered_command in real_execution_sandbox_rendered_command_scaffolds:
        command_render_plan_id = clean(
            rendered_command.get(
                "real_execution_sandbox_command_render_plan_scaffold_id"
            )
        )
        if command_render_plan_id and command_render_plan_id in command_render_plan_ids:
            rendered_command_plan_matches += 1
        else:
            rendered_command_orphans += 1

    return {
        "real_execution_sandbox_rendered_command_scaffold_command_render_plan_matches": (
            rendered_command_plan_matches
        ),
        "real_execution_sandbox_rendered_command_scaffold_orphans": (
            rendered_command_orphans
        ),
        "real_execution_sandbox_rendered_command_scaffold_linkage_complete": bool(
            real_execution_sandbox_rendered_command_scaffolds
        )
        and rendered_command_orphans == 0,
    }


def _real_execution_sandbox_rendered_command_validation_scaffold_linkage_summary(
    *,
    real_execution_sandbox_rendered_command_scaffolds: list[Mapping[str, Any]],
    real_execution_sandbox_rendered_command_validation_scaffolds: list[
        Mapping[str, Any]
    ...
    rendered_command_scaffold_ids = {
        clean(item.get("real_execution_sandbox_rendered_command_scaffold_id"))
        for item in real_execution_sandbox_rendered_command_scaffolds
        if clean(item.get("real_execution_sandbox_rendered_command_scaffold_id"))
    }

    validation_rendered_command_matches = 0
    validation_orphans = 0

    for validation in real_execution_sandbox_rendered_command_validation_scaffolds:
        rendered_scaffold_id = clean(
            validation.get("real_execution_sandbox_rendered_command_scaffold_id")
        )
        if rendered_scaffold_id and rendered_scaffold_id in rendered_command_scaffold_ids:
            validation_rendered_command_matches += 1
        else:
            validation_orphans += 1

    return {
        "real_execution_sandbox_rendered_command_validation_scaffold_rendered_command_matches": (
            validation_rendered_command_matches
        ),
        "real_execution_sandbox_rendered_command_validation_scaffold_orphans": (
            validation_orphans
        ),
