"""Build fail-closed sandbox command render plan scaffold records.

This artifact prepares a blocked command render plan scaffold from a valid
...
REQUIRED_INPUT_PLAN_TRUE_FLAGS = [
...
REQUIRED_INPUT_PLAN_FALSE_FLAGS = [
...
EXPECTED_INPUT_PLAN_STRINGS = {
    "schema_version": EXPECTED_SANDBOX_INPUT_MATERIALIZATION_PLAN_SCAFFOLD_SCHEMA_VERSION,
    ...
    "reason": "sandbox_input_materialization_plan_scaffold_defined_blocked_not_runnable",
}
...
def _require_bool(record: Mapping[str, Any], key: str, expected: bool) -> None:
    ...
    if value is not expected:
        ...
        raise ValueError(
            "source sandbox input materialization plan scaffold requires "
            f"{key}={expected_text}"
        )


def _validate_sandbox_input_materialization_plan_scaffold(
    ...
    for key, expected in EXPECTED_INPUT_PLAN_STRINGS.items():
    ...
    for key in REQUIRED_INPUT_PLAN_TRUE_FLAGS:
        _require_bool(record, key, True)

    for key in REQUIRED_INPUT_PLAN_FALSE_FLAGS:
    ...
    if missing:
        raise ValueError(
            "source sandbox input materialization plan scaffold missing "
            "required field(s): "
            + ",".join(missing)
        )
    ...
    if _safe_list(record.get("sandbox_request_allowed_input_paths")):
        raise ValueError(
            "source sandbox input materialization plan scaffold must not "
            "allow input paths"
        )
...
def build_real_execution_sandbox_command_render_plan_scaffold_record(
    sandbox_input_materialization_plan_scaffold: Mapping[str, Any],
    *,
    source: str = "real-execution-sandbox-command-render-plan-scaffold",
) -> dict[str, Any]:
    """Build blocked sandbox command render plan scaffold record."""
    _validate_sandbox_input_materialization_plan_scaffold(
        sandbox_input_materialization_plan_scaffold
    )

    input_plan_id = _clean(
        sandbox_input_materialization_plan_scaffold.get(
    ...
    rendered_command_id = _clean(
        sandbox_input_materialization_plan_scaffold.get("rendered_command_id")
    )
    proposal_id = _clean(
        sandbox_input_materialization_plan_scaffold.get("proposal_id")
    )

    command_render_plan_id = _stable_id(
        "replay-retry-real-execution-sandbox-command-render-plan-scaffold",
        input_plan_id,
        preparation_id,
        workspace_plan_id,
        materialization_id,
    ...
    payload = {
        ...
        "plan_id": _clean(sandbox_input_materialization_plan_scaffold.get("plan_id")),
        "proposal_id": proposal_id,
        "approval_id": _clean(
            sandbox_input_materialization_plan_scaffold.get("approval_id")
        ),
        "decision_mode": _clean(
            sandbox_input_materialization_plan_scaffold.get("decision_mode")
        )
        or "manual",
        "timeout_profile": _clean(
            sandbox_input_materialization_plan_scaffold.get("timeout_profile")
        )
        or "standard",
        "schema_version": SANDBOX_COMMAND_RENDER_PLAN_SCAFFOLD_SCHEMA_VERSION,
        "source_input_materialization_plan_scaffold_schema_version": _clean(
            sandbox_input_materialization_plan_scaffold.get("schema_version")
        ),
        ...
        "sandbox_production_write_policy": "deny",
        "sandbox_external_side_effect_policy": "deny",
        "source_input_materialization_plan_scaffold_status": _clean(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_input_materialization_plan_scaffold_status"
            )
        ),
        "source_input_materialization_plan_scaffold_exists": bool(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_input_materialization_plan_scaffold_exists"
            )
        ),
        "source_input_materialization_plan_scaffold_fail_closed": bool(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_input_materialization_plan_scaffold_fail_closed"
            )
        ),
        "source_input_materialization_plan_scaffold_deny_by_default": bool(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_input_materialization_plan_scaffold_deny_by_default"
            )
        ),
        "source_input_materialization_plan_generation_enabled": bool(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_input_materialization_plan_generation_enabled"
            )
        ),
        "source_input_materialization_plan_materialized": bool(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_input_materialization_plan_materialized"
            )
        ),
        "source_input_materialization_plan_executable": bool(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_input_materialization_plan_executable"
            )
        ),
        ...
        "source_inputs_materialized": bool(
            sandbox_input_materialization_plan_scaffold.get(
                "sandbox_inputs_materialized"
            )
        ),
        ...
        "source_sandbox_execution_enabled": bool(
            sandbox_input_materialization_plan_scaffold.get("sandbox_execution_enabled")
        ),
        ...
        "source_execution_performed": bool(
            sandbox_input_materialization_plan_scaffold.get("execution_performed")
        ),
        "source_subprocess_invoked": bool(
            sandbox_input_materialization_plan_scaffold.get("subprocess_invoked")
        ),
        "source_real_execution_enabled": bool(
            sandbox_input_materialization_plan_scaffold.get("real_execution_enabled")
        ),
    ...
    return {
        "type": SANDBOX_COMMAND_RENDER_PLAN_SCAFFOLD_TYPE,
        **payload,
        "source": source,
        "payload": payload,
    }
...
def _matches_filters(
    ...
    *,
    ...
    input_materialization_plan_id: str,
) -> bool:
...
def _find_existing_command_render_plan(
...
async def build_real_execution_sandbox_command_render_plan_scaffold_records(
    args: argparse.Namespace,
) -> list[dict[str, Any]]:
    db_path = str(args.db_path or config.crdt_db_path)
    source = (
        _clean(getattr(args, "source", ""))
        or "real-execution-sandbox-command-render-plan-scaffold"
    )
    proposal_id = _clean(getattr(args, "proposal_id", ""))
    rendered_command_id = _clean(getattr(args, "rendered_command_id", ""))
    input_materialization_plan_id = _clean(
        getattr(
            args,
            "real_execution_sandbox_input_materialization_plan_scaffold_id",
            "",
    ...
    input_plans = [
        ...
        and _matches_filters(
            ...
            proposal_id=proposal_id,
            rendered_command_id=rendered_command_id,
            input_materialization_plan_id=input_materialization_plan_id,
        )
    ]

    results: list[dict[str, Any]] = []
    for input_plan in input_plans:
        current_input_plan_id = _clean(
            input_plan.get(
                "real_execution_sandbox_input_materialization_plan_scaffold_id"
            )
        )
        if _find_existing_command_render_plan(
            records,
            input_materialization_plan_id=current_input_plan_id,
        ):
            logger.info(
                "Skipping duplicate sandbox command render plan scaffold: "
                "input_materialization_plan_id=%s",
                current_input_plan_id,
            )
            continue
        ...
        logger.info(
            "Published sandbox command render plan scaffold: "
            "command_render_plan_id=%s status=%s plan_generation_enabled=%s "
            "command_rendering_enabled=%s sandbox_execution_enabled=%s",
            record.get("real_execution_sandbox_command_render_plan_scaffold_id"),
            record.get("sandbox_command_render_plan_scaffold_status"),
    ...
    logger.info(
        "Sandbox command render plan scaffold builder completed: "
        "command_render_plans=%s",
        len(results),
    )
    return results
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build blocked sandbox command render plan scaffold records.",
    )
    ...
    parser.add_argument(
        "--real-execution-sandbox-input-materialization-plan-scaffold-id",
        default="",
    )
    parser.add_argument(
        "--source",
        default="real-execution-sandbox-command-render-plan-scaffold",
    )
...
async def _async_main() -> None:
    ...
    results = await build_real_execution_sandbox_command_render_plan_scaffold_records(
        args
    )
    ...
    if args.json:
        ...
        print(
            "Sandbox command render plan scaffold builder completed: "
            f"command_render_plans={len(results)}"
        )
