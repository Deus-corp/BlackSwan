...
def validate_runtime_records(records: Iterable[Any]) -> list[dict[str, Any]]:
    ...
    for record in records or []:
        ...
        if (
            ...
            result = (
                validate_replay_lifecycle_retry_real_execution_sandbox_workspace_plan_scaffold(
                    record
                )
            )
            results.append(
        ...
        if (
            ...
            result = (
                validate_replay_lifecycle_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold(
                    record
                )
            )
            results.append(
        ...
        if (
            ...
            result = (
                validate_replay_lifecycle_retry_real_execution_sandbox_input_materialization_plan_scaffold(
                    record
                )
            )
            results.append(
        ...
        if (
            ...
            result = (
                validate_replay_lifecycle_retry_real_execution_sandbox_command_render_plan_scaffold(
                    record
                )
            )
            results.append(
        ...
        if (
            ...
            result = (
                validate_replay_lifecycle_retry_real_execution_sandbox_rendered_command_scaffold(
                    record
                )
            )
            results.append(
        ...
        if (
            ...
            result = (
                validate_replay_lifecycle_retry_real_execution_sandbox_rendered_command_validation_scaffold(
                    record
                )
            )
            results.append(
...
def validate_replay_lifecycle_retry_real_execution_adapter_request_schema(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed real execution adapter request schema records."""
    reasons: list[str] = []
    ...
    contract_schema_version = str(
        record.get("adapter_contract_schema_version") or ""
    ).strip()
    result_schema_version = str(
        record.get("adapter_result_schema_version") or ""
    ).strip()
    schema_status = str(record.get("adapter_request_schema_status") or "").strip()
    schema_kind = str(record.get("adapter_request_schema_kind") or "").strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()

    request_required_fields = record.get("request_required_fields")
    required_gate_fields = record.get("required_gate_fields")
    request_statuses = record.get("request_statuses")
    request_execution_levels = record.get("request_execution_levels")
    request_generation_rules = record.get("request_generation_rules")
    default_rejection_reasons = record.get("default_rejection_reasons")

    source_contract_status = str(record.get("source_contract_status") or "").strip()
    source_expected_count = record.get("source_repair_targets_expected_count")
    source_verified_count = record.get("source_repair_targets_verified_count")
    ...
    if schema_version != "real-execution-adapter-request-schema-scaffold/v1":
        reasons.append("invalid_real_execution_adapter_request_schema_scaffold_version")
    if request_schema_version != "real-execution-adapter-request/v1":
        reasons.append("invalid_real_execution_adapter_request_schema_version")
    if contract_schema_version != "real-execution-adapter-contract/v1":
        reasons.append("invalid_real_execution_adapter_contract_schema_version")
    if result_schema_version != "real-execution-adapter-result/v1":
        reasons.append("invalid_real_execution_adapter_result_schema_version")
    if schema_status != "defined":
        reasons.append("real_execution_adapter_request_schema_status_must_be_defined")
    if schema_kind != "policy_gated_real_execution_adapter_request":
        reasons.append("invalid_real_execution_adapter_request_schema_kind")
    if next_action != "prepare_capability_registry_and_policy_matrix":
        reasons.append("invalid_real_execution_adapter_request_schema_next_action")
    if reason != "real_execution_adapter_request_schema_defined_not_runnable":
        reasons.append("invalid_real_execution_adapter_request_schema_reason")
    ...
    for flag_name in (
        ...
        "unsupported_execution_level_rejected",
        "missing_operator_authorization_rejected",
        "missing_approval_lineage_rejected",
        "missing_final_gate_rejected",
        "missing_dry_run_envelope_rejected",
        "missing_rollback_plan_rejected",
        "missing_post_execution_evidence_rejected",
        "orphaned_contract_rejected",
    ...
    if source_contract_status != "defined":
        reasons.append("source_contract_status_must_be_defined")
    ...
    if not isinstance(request_required_fields, list):
        reasons.append("request_required_fields_must_be_list")
        request_required_fields = []
    if not isinstance(required_gate_fields, list):
        reasons.append("required_gate_fields_must_be_list")
        required_gate_fields = []
    if not isinstance(request_statuses, list):
        reasons.append("request_statuses_must_be_list")
        request_statuses = []
    if not isinstance(request_execution_levels, list):
        reasons.append("request_execution_levels_must_be_list")
        request_execution_levels = []
    if not isinstance(request_generation_rules, list):
        reasons.append("request_generation_rules_must_be_list")
        request_generation_rules = []
    if not isinstance(default_rejection_reasons, list):
        reasons.append("default_rejection_reasons_must_be_list")
    ...
    for field in (
        ...
        if field not in request_required_fields:
            reasons.append(f"missing_request_required_field:{field}")
    ...
    for status in ("defined", "rejected", "blocked", "expired", "superseded"):
        if status not in request_statuses:
            reasons.append(f"missing_request_status:{status}")
    ...
    for level in (
        ...
        if level not in request_execution_levels:
            reasons.append(f"missing_request_execution_level:{level}")
    ...
    for rule in (
        ...
        if rule not in request_generation_rules:
            reasons.append(f"missing_request_generation_rule:{rule}")
    ...
    for rejection_reason in (
        ...
        if rejection_reason not in default_rejection_reasons:
            reasons.append(f"missing_default_rejection_reason:{rejection_reason}")

    return {
        "type": "security_validation_result",
        "record_type": "replay_lifecycle_retry_real_execution_adapter_request_schema",
        "valid": not reasons,
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": schema_id or contract_id,
        "adapter_request_schema_status": schema_status or "unknown",
        "schema_version": schema_version or "unknown",
        "adapter_request_schema_version": request_schema_version or "unknown",
        "adapter_contract_schema_version": contract_schema_version or "unknown",
        "adapter_result_schema_version": result_schema_version or "unknown",
        "adapter_request_schema_exists": bool(record.get("adapter_request_schema_exists")),
        "adapter_contract_exists": bool(record.get("adapter_contract_exists")),
        "adapter_result_schema_exists": bool(record.get("adapter_result_schema_exists")),
        "fail_closed_default": bool(record.get("fail_closed_default")),
        "deny_by_default": bool(record.get("deny_by_default")),
        "unknown_capability_rejected": bool(record.get("unknown_capability_rejected")),
        "unknown_policy_rejected": bool(record.get("unknown_policy_rejected")),
        "request_generation_enabled": bool(record.get("request_generation_enabled")),
        "request_execution_enabled": bool(record.get("request_execution_enabled")),
        "adapter_implementation_enabled": bool(record.get("adapter_implementation_enabled")),
        "adapter_result_generation_enabled": bool(record.get("adapter_result_generation_enabled")),
        "sandbox_execution_enabled": bool(record.get("sandbox_execution_enabled")),
        "policy_gated_real_execution_enabled": bool(record.get("policy_gated_real_execution_enabled")),
        "execution_performed": bool(record.get("execution_performed")),
        "subprocess_invoked": bool(record.get("subprocess_invoked")),
        "real_execution_enabled": bool(record.get("real_execution_enabled")),
        "external_side_effects_performed": bool(record.get("external_side_effects_performed")),
        "production_paths_mutated": bool(record.get("production_paths_mutated")),
        "production_secrets_accessed": bool(record.get("production_secrets_accessed")),
        "source_contract_status": source_contract_status or "unknown",
        "source_repair_outcome_verified": bool(record.get("source_repair_outcome_verified")),
        "source_repair_targets_expected_count": source_expected_count,
        "source_repair_targets_verified_count": source_verified_count,
        "recommended_next_action": next_action or "unknown",
        "reason": reason or "unknown",
...
def validate_replay_lifecycle_retry_real_execution_sandbox_adapter_request_preflight(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox adapter request preflight records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(record.get("sandbox_adapter_request_preflight_status") or "").strip()
    kind = str(record.get("sandbox_adapter_request_preflight_kind") or "").strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()
    ...
    if schema_version != "real-execution-sandbox-adapter-request-preflight/v1":
        reasons.append("invalid_sandbox_adapter_request_preflight_schema_version")
    if status != "blocked":
        reasons.append("sandbox_adapter_request_preflight_status_must_be_blocked")
    if kind != "fail_closed_sandbox_adapter_request_preflight":
        reasons.append("invalid_sandbox_adapter_request_preflight_kind")
    if next_action != "surface_sandbox_adapter_request_preflight_observability":
        reasons.append("invalid_sandbox_adapter_request_preflight_next_action")
    if reason != "sandbox_adapter_request_preflight_defined_blocked_not_runnable":
        reasons.append("invalid_sandbox_adapter_request_preflight_reason")
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": preflight_id or scaffold_id,
        "schema_version": schema_version or "unknown",
        "sandbox_adapter_request_preflight_status": status or "unknown",
        "sandbox_adapter_request_preflight_fail_closed": bool(
            record.get("sandbox_adapter_request_preflight_fail_closed")
        ),
        "sandbox_adapter_request_preflight_deny_by_default": bool(
            record.get("sandbox_adapter_request_preflight_deny_by_default")
        ),
        "sandbox_adapter_request_generation_enabled": bool(
            record.get("sandbox_adapter_request_generation_enabled")
        ),
...
def validate_replay_lifecycle_retry_real_execution_sandbox_request_envelope_scaffold(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox request envelope scaffold records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(record.get("sandbox_request_envelope_scaffold_status") or "").strip()
    kind = str(record.get("sandbox_request_envelope_scaffold_kind") or "").strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()
    ...
    if schema_version != "real-execution-sandbox-request-envelope-scaffold/v1":
        reasons.append("invalid_sandbox_request_envelope_scaffold_schema_version")
    if status != "blocked":
        reasons.append("sandbox_request_envelope_scaffold_status_must_be_blocked")
    if kind != "fail_closed_sandbox_request_envelope_scaffold":
        reasons.append("invalid_sandbox_request_envelope_scaffold_kind")
    if next_action != "surface_sandbox_request_envelope_scaffold_observability":
        reasons.append("invalid_sandbox_request_envelope_scaffold_next_action")
    if reason != "sandbox_request_envelope_scaffold_defined_blocked_not_runnable":
        reasons.append("invalid_sandbox_request_envelope_scaffold_reason")
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": envelope_id or preflight_id,
        "schema_version": schema_version or "unknown",
        "sandbox_request_envelope_scaffold_status": status or "unknown",
        "sandbox_request_envelope_scaffold_fail_closed": bool(
            record.get("sandbox_request_envelope_scaffold_fail_closed")
        ),
        "sandbox_request_envelope_scaffold_deny_by_default": bool(
            record.get("sandbox_request_envelope_scaffold_deny_by_default")
        ),
...
def validate_replay_lifecycle_retry_real_execution_sandbox_workspace_plan_scaffold(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox workspace plan scaffold records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}

    workspace_plan_id = str(
        record.get("real_execution_sandbox_workspace_plan_scaffold_id") or ""
    ).strip()
    materialization_id = str(
        record.get("real_execution_sandbox_materialization_preflight_scaffold_id")
        or ""
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(record.get("sandbox_workspace_plan_scaffold_status") or "").strip()
    kind = str(record.get("sandbox_workspace_plan_scaffold_kind") or "").strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()

    if not workspace_plan_id:
        reasons.append("missing_real_execution_sandbox_workspace_plan_scaffold_id")
    if not materialization_id:
        reasons.append(
            "missing_real_execution_sandbox_materialization_preflight_scaffold_id"
        )
    ...
    if schema_version != "real-execution-sandbox-workspace-plan-scaffold/v1":
        reasons.append("invalid_sandbox_workspace_plan_scaffold_schema_version")
    if status != "blocked":
        reasons.append("sandbox_workspace_plan_scaffold_status_must_be_blocked")
    if kind != "fail_closed_sandbox_workspace_plan_scaffold":
        reasons.append("invalid_sandbox_workspace_plan_scaffold_kind")
    if next_action != "surface_sandbox_workspace_plan_scaffold_observability":
        reasons.append("invalid_sandbox_workspace_plan_scaffold_next_action")
    if reason != "sandbox_workspace_plan_scaffold_defined_blocked_not_runnable":
        reasons.append("invalid_sandbox_workspace_plan_scaffold_reason")
    ...
    expected_strings = {
        ...
        "source_materialization_preflight_scaffold_status": "blocked",
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
    ...
    required_true_flags = (
        "sandbox_workspace_plan_scaffold_exists",
        "sandbox_workspace_plan_scaffold_fail_closed",
        "sandbox_workspace_plan_scaffold_deny_by_default",
        "sandbox_workspace_plan_requires_materialization_preflight",
        "sandbox_workspace_plan_requires_envelope_scaffold",
        "sandbox_workspace_plan_requires_request_preflight",
        "sandbox_workspace_plan_requires_policy_matrix",
        "sandbox_workspace_plan_requires_known_capability",
        "sandbox_workspace_plan_requires_known_policy",
        "sandbox_workspace_plan_requires_operator_authorization",
        "sandbox_workspace_plan_requires_approval_lineage",
        "sandbox_workspace_plan_requires_final_gate",
        "sandbox_workspace_plan_requires_dry_run_envelope",
        "sandbox_workspace_plan_requires_rollback_plan",
        "sandbox_workspace_plan_requires_post_execution_evidence",
        "sandbox_workspace_plan_rejects_unknown_capability",
        "sandbox_workspace_plan_rejects_unknown_policy",
        "sandbox_workspace_plan_rejects_orphans",
        "sandbox_workspace_plan_rejects_stale_records",
        "source_materialization_preflight_scaffold_exists",
        "source_materialization_preflight_scaffold_fail_closed",
        "source_materialization_preflight_scaffold_deny_by_default",
    )
    for flag_name in required_true_flags:
        if not bool(record.get(flag_name)):
            reasons.append(f"{flag_name}_must_be_true")
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": workspace_plan_id or materialization_id,
        "schema_version": schema_version or "unknown",
        "sandbox_workspace_plan_scaffold_status": status or "unknown",
        "sandbox_workspace_plan_scaffold_fail_closed": bool(
            record.get("sandbox_workspace_plan_scaffold_fail_closed")
        ),
        "sandbox_workspace_plan_scaffold_deny_by_default": bool(
            record.get("sandbox_workspace_plan_scaffold_deny_by_default")
        ),
        "sandbox_workspace_plan_generation_enabled": bool(
            record.get("sandbox_workspace_plan_generation_enabled")
        ),
        "sandbox_workspace_plan_materialized": bool(
            record.get("sandbox_workspace_plan_materialized")
        ),
        "sandbox_workspace_plan_executable": bool(
            record.get("sandbox_workspace_plan_executable")
        ),
        "sandbox_workspace_directory_creation_enabled": bool(
            record.get("sandbox_workspace_directory_creation_enabled")
        ),
        "sandbox_workspace_created": bool(record.get("sandbox_workspace_created")),
        "sandbox_workspace_cleanup_registered": bool(
            record.get("sandbox_workspace_cleanup_registered")
        ),
        "sandbox_materialization_preflight_enabled": bool(
            record.get("sandbox_materialization_preflight_enabled")
        ),
...
def validate_replay_lifecycle_retry_real_execution_sandbox_workspace_preparation_preflight_scaffold(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox workspace preparation preflight scaffold records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}

    preparation_preflight_id = str(
        record.get(
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(
        record.get("sandbox_workspace_preparation_preflight_scaffold_status") or ""
    ).strip()
    kind = str(
        record.get("sandbox_workspace_preparation_preflight_scaffold_kind") or ""
    ).strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()

    if not preparation_preflight_id:
        reasons.append(
            "missing_real_execution_sandbox_workspace_preparation_preflight_scaffold_id"
        )
    if not workspace_plan_id:
        reasons.append("missing_real_execution_sandbox_workspace_plan_scaffold_id")
    if not materialization_id:
    ...
    if (
        ...
        reasons.append(
            "invalid_sandbox_workspace_preparation_preflight_scaffold_schema_version"
        )
    if status != "blocked":
        reasons.append(
            "sandbox_workspace_preparation_preflight_scaffold_status_must_be_blocked"
        )
    if kind != "fail_closed_sandbox_workspace_preparation_preflight_scaffold":
        reasons.append(
            "invalid_sandbox_workspace_preparation_preflight_scaffold_kind"
        )
    if (
        next_action
        ...
        reasons.append(
            "invalid_sandbox_workspace_preparation_preflight_scaffold_next_action"
        )
    if (
        reason
        != "sandbox_workspace_preparation_preflight_scaffold_defined_blocked_not_runnable"
    ):
        reasons.append("invalid_sandbox_workspace_preparation_preflight_scaffold_reason")
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": preparation_preflight_id or workspace_plan_id,
        "schema_version": schema_version or "unknown",
        "sandbox_workspace_preparation_preflight_scaffold_status": status or "unknown",
        "sandbox_workspace_preparation_preflight_scaffold_fail_closed": bool(
            record.get("sandbox_workspace_preparation_preflight_scaffold_fail_closed")
        ),
        "sandbox_workspace_preparation_preflight_scaffold_deny_by_default": bool(
            record.get(
                "sandbox_workspace_preparation_preflight_scaffold_deny_by_default"
            )
        ),
        "sandbox_workspace_preparation_preflight_enabled": bool(
            record.get("sandbox_workspace_preparation_preflight_enabled")
        ),
        "sandbox_workspace_preparation_preflight_passed": bool(
            record.get("sandbox_workspace_preparation_preflight_passed")
        ),
        "sandbox_workspace_plan_generation_enabled": bool(
            record.get("sandbox_workspace_plan_generation_enabled")
        ),
...
def validate_replay_lifecycle_retry_real_execution_sandbox_input_materialization_plan_scaffold(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox input materialization plan scaffold records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(
        record.get("sandbox_input_materialization_plan_scaffold_status") or ""
    ).strip()
    kind = str(
        record.get("sandbox_input_materialization_plan_scaffold_kind") or ""
    ).strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()
    ...
    if not input_plan_id:
        reasons.append(
            "missing_real_execution_sandbox_input_materialization_plan_scaffold_id"
        )
    ...
    if schema_version != "real-execution-sandbox-input-materialization-plan-scaffold/v1":
        reasons.append(
            "invalid_sandbox_input_materialization_plan_scaffold_schema_version"
        )
    if status != "blocked":
        reasons.append(
            "sandbox_input_materialization_plan_scaffold_status_must_be_blocked"
        )
    if kind != "fail_closed_sandbox_input_materialization_plan_scaffold":
        reasons.append("invalid_sandbox_input_materialization_plan_scaffold_kind")
    if next_action != "surface_sandbox_input_materialization_plan_scaffold_observability":
        reasons.append(
            "invalid_sandbox_input_materialization_plan_scaffold_next_action"
        )
    if reason != "sandbox_input_materialization_plan_scaffold_defined_blocked_not_runnable":
        reasons.append("invalid_sandbox_input_materialization_plan_scaffold_reason")
    ...
    expected_strings = {
        ...
        "source_workspace_preparation_preflight_scaffold_status": "blocked",
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_workspace_plan_strategy": "explicit_plan_only",
        "sandbox_workspace_preparation_strategy": (
            "preflight_only_no_directory_creation"
    ...
    required_true_flags = (
        "sandbox_input_materialization_plan_scaffold_exists",
        ...
        "sandbox_input_materialization_plan_requires_workspace_plan",
        "sandbox_input_materialization_plan_requires_materialization_preflight",
        "sandbox_input_materialization_plan_requires_envelope_scaffold",
        "sandbox_input_materialization_plan_requires_request_preflight",
        "sandbox_input_materialization_plan_requires_policy_matrix",
        "sandbox_input_materialization_plan_requires_known_capability",
        "sandbox_input_materialization_plan_requires_known_policy",
        "sandbox_input_materialization_plan_requires_operator_authorization",
        "sandbox_input_materialization_plan_requires_approval_lineage",
        "sandbox_input_materialization_plan_requires_final_gate",
        "sandbox_input_materialization_plan_requires_dry_run_envelope",
        "sandbox_input_materialization_plan_requires_rollback_plan",
        "sandbox_input_materialization_plan_requires_post_execution_evidence",
        "sandbox_input_materialization_plan_rejects_unknown_capability",
        "sandbox_input_materialization_plan_rejects_unknown_policy",
        "sandbox_input_materialization_plan_rejects_orphans",
        "sandbox_input_materialization_plan_rejects_stale_records",
        "source_workspace_preparation_preflight_scaffold_exists",
        "source_workspace_preparation_preflight_scaffold_fail_closed",
        "source_workspace_preparation_preflight_scaffold_deny_by_default",
    )
    for flag_name in required_true_flags:
        if not bool(record.get(flag_name)):
            reasons.append(f"{flag_name}_must_be_true")
    ...
    required_false_flags = (
        ...
        "production_paths_mutated",
        "production_secrets_accessed",
        "source_workspace_preparation_preflight_enabled",
        "source_workspace_preparation_preflight_passed",
        "source_directory_creation_enabled",
        "source_workspace_created",
        "source_cleanup_registered",
        "source_workspace_creation_enabled",
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": input_plan_id or preparation_id,
        "schema_version": schema_version or "unknown",
        "sandbox_input_materialization_plan_scaffold_status": status or "unknown",
        "sandbox_input_materialization_plan_scaffold_fail_closed": bool(
            record.get("sandbox_input_materialization_plan_scaffold_fail_closed")
        ),
        "sandbox_input_materialization_plan_scaffold_deny_by_default": bool(
            record.get("sandbox_input_materialization_plan_scaffold_deny_by_default")
        ),
        "sandbox_input_materialization_plan_generation_enabled": bool(
            record.get("sandbox_input_materialization_plan_generation_enabled")
        ),
        "sandbox_input_materialization_plan_materialized": bool(
            record.get("sandbox_input_materialization_plan_materialized")
        ),
        "sandbox_input_materialization_plan_executable": bool(
            record.get("sandbox_input_materialization_plan_executable")
        ),
        "sandbox_input_materialization_enabled": bool(
            record.get("sandbox_input_materialization_enabled")
        ),
        "sandbox_inputs_materialized": bool(record.get("sandbox_inputs_materialized")),
        "sandbox_workspace_preparation_preflight_enabled": bool(
            record.get("sandbox_workspace_preparation_preflight_enabled")
        ),
        "sandbox_workspace_preparation_preflight_passed": bool(
...
def validate_replay_lifecycle_retry_real_execution_sandbox_command_render_plan_scaffold(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox command render plan scaffold records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(record.get("sandbox_command_render_plan_scaffold_status") or "").strip()
    kind = str(record.get("sandbox_command_render_plan_scaffold_kind") or "").strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()

    if not command_render_plan_id:
        reasons.append("missing_real_execution_sandbox_command_render_plan_scaffold_id")
    if not input_plan_id:
        reasons.append(
            "missing_real_execution_sandbox_input_materialization_plan_scaffold_id"
        )
    ...
    if schema_version != "real-execution-sandbox-command-render-plan-scaffold/v1":
        reasons.append("invalid_sandbox_command_render_plan_scaffold_schema_version")
    if status != "blocked":
        reasons.append("sandbox_command_render_plan_scaffold_status_must_be_blocked")
    if kind != "fail_closed_sandbox_command_render_plan_scaffold":
        reasons.append("invalid_sandbox_command_render_plan_scaffold_kind")
    if next_action != "surface_sandbox_command_render_plan_scaffold_observability":
        reasons.append("invalid_sandbox_command_render_plan_scaffold_next_action")
    if reason != "sandbox_command_render_plan_scaffold_defined_blocked_not_runnable":
        reasons.append("invalid_sandbox_command_render_plan_scaffold_reason")
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": command_render_plan_id or input_plan_id,
        "schema_version": schema_version or "unknown",
        "sandbox_command_render_plan_scaffold_status": status or "unknown",
        "sandbox_command_render_plan_scaffold_fail_closed": bool(
            record.get("sandbox_command_render_plan_scaffold_fail_closed")
        ),
        "sandbox_command_render_plan_scaffold_deny_by_default": bool(
            record.get("sandbox_command_render_plan_scaffold_deny_by_default")
        ),
        "sandbox_command_render_plan_generation_enabled": bool(
            record.get("sandbox_command_render_plan_generation_enabled")
        ),
        "sandbox_command_render_plan_materialized": bool(
            record.get("sandbox_command_render_plan_materialized")
        ),
        "sandbox_command_render_plan_executable": bool(
            record.get("sandbox_command_render_plan_executable")
        ),
        "sandbox_command_rendering_enabled": bool(
            record.get("sandbox_command_rendering_enabled")
        ),
        "sandbox_command_rendered": bool(record.get("sandbox_command_rendered")),
        "sandbox_rendered_command_validated": bool(
            record.get("sandbox_rendered_command_validated")
        ),
        "sandbox_input_materialization_plan_generation_enabled": bool(
            record.get("sandbox_input_materialization_plan_generation_enabled")
        ),
...
def validate_replay_lifecycle_retry_real_execution_sandbox_rendered_command_scaffold(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox rendered command scaffold records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}

    rendered_scaffold_id = str(
        record.get("real_execution_sandbox_rendered_command_scaffold_id") or ""
    ).strip()
    command_render_plan_id = str(
        record.get("real_execution_sandbox_command_render_plan_scaffold_id") or ""
    ).strip()
    input_plan_id = str(
        record.get("real_execution_sandbox_input_materialization_plan_scaffold_id")
        or ""
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(record.get("sandbox_rendered_command_scaffold_status") or "").strip()
    kind = str(record.get("sandbox_rendered_command_scaffold_kind") or "").strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()

    if not rendered_scaffold_id:
        reasons.append("missing_real_execution_sandbox_rendered_command_scaffold_id")
    if not command_render_plan_id:
        reasons.append(
            "missing_real_execution_sandbox_command_render_plan_scaffold_id"
        )
    if not input_plan_id:
        reasons.append(
            "missing_real_execution_sandbox_input_materialization_plan_scaffold_id"
    ...
    if schema_version != "real-execution-sandbox-rendered-command-scaffold/v1":
        reasons.append("invalid_sandbox_rendered_command_scaffold_schema_version")
    if status != "blocked":
        reasons.append("sandbox_rendered_command_scaffold_status_must_be_blocked")
    if kind != "fail_closed_sandbox_rendered_command_scaffold":
        reasons.append("invalid_sandbox_rendered_command_scaffold_kind")
    if next_action != "surface_sandbox_rendered_command_scaffold_observability":
        reasons.append("invalid_sandbox_rendered_command_scaffold_next_action")
    if reason != "sandbox_rendered_command_scaffold_defined_blocked_not_runnable":
        reasons.append("invalid_sandbox_rendered_command_scaffold_reason")
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": rendered_scaffold_id or command_render_plan_id,
        "schema_version": schema_version or "unknown",
        "sandbox_rendered_command_scaffold_status": status or "unknown",
        "sandbox_rendered_command_scaffold_fail_closed": bool(
            record.get("sandbox_rendered_command_scaffold_fail_closed")
        ),
        "sandbox_rendered_command_scaffold_deny_by_default": bool(
            record.get("sandbox_rendered_command_scaffold_deny_by_default")
        ),
        "sandbox_rendered_command_generation_enabled": bool(
            record.get("sandbox_rendered_command_generation_enabled")
        ),
        "sandbox_rendered_command_materialized": bool(
            record.get("sandbox_rendered_command_materialized")
        ),
        "sandbox_rendered_command_executable": bool(
            record.get("sandbox_rendered_command_executable")
        ),
        "sandbox_rendered_command_validated": bool(
            record.get("sandbox_rendered_command_validated")
        ),
...
def validate_replay_lifecycle_retry_real_execution_sandbox_rendered_command_validation_scaffold(
    record: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate fail-closed sandbox rendered command validation scaffold records."""
    reasons: list[str] = []
    payload = record.get("payload")
    payload_mapping = payload if isinstance(payload, Mapping) else {}

    validation_id = str(
        record.get("real_execution_sandbox_rendered_command_validation_scaffold_id")
        or ""
    ).strip()
    rendered_scaffold_id = str(
        record.get("real_execution_sandbox_rendered_command_scaffold_id") or ""
    ...
    schema_version = str(record.get("schema_version") or "").strip()
    status = str(
        record.get("sandbox_rendered_command_validation_scaffold_status") or ""
    ).strip()
    kind = str(
        record.get("sandbox_rendered_command_validation_scaffold_kind") or ""
    ).strip()
    next_action = str(record.get("recommended_next_action") or "").strip()
    reason = str(record.get("reason") or "").strip()
    ...
    if not validation_id:
        reasons.append(
            "missing_real_execution_sandbox_rendered_command_validation_scaffold_id"
        )
    if not rendered_scaffold_id:
        reasons.append("missing_real_execution_sandbox_rendered_command_scaffold_id")
    if not command_render_plan_id:
    ...
    if schema_version != "real-execution-sandbox-rendered-command-validation-scaffold/v1":
        reasons.append(
            "invalid_sandbox_rendered_command_validation_scaffold_schema_version"
        )
    if status != "blocked":
        reasons.append(
            "sandbox_rendered_command_validation_scaffold_status_must_be_blocked"
        )
    if kind != "fail_closed_sandbox_rendered_command_validation_scaffold":
        reasons.append("invalid_sandbox_rendered_command_validation_scaffold_kind")
    if (
        next_action
        != "surface_sandbox_rendered_command_validation_scaffold_observability"
    ):
        reasons.append(
            "invalid_sandbox_rendered_command_validation_scaffold_next_action"
        )
    if (
        reason
        != "sandbox_rendered_command_validation_scaffold_defined_blocked_not_runnable"
    ):
        reasons.append("invalid_sandbox_rendered_command_validation_scaffold_reason")

    expected_strings = {
        "source_rendered_command_scaffold_schema_version": (
        ...
        "source_rendered_command_scaffold_status": "blocked",
        "sandbox_workspace_strategy": "ephemeral_temp_workspace",
        "sandbox_workspace_plan_strategy": "explicit_plan_only",
        "sandbox_workspace_preparation_strategy": (
            "preflight_only_no_directory_creation"
    ...
    required_true_flags = (
        "sandbox_rendered_command_validation_scaffold_exists",
        ...
        "sandbox_rendered_command_validation_requires_command_render_plan",
        "sandbox_rendered_command_validation_requires_input_materialization_plan",
        "sandbox_rendered_command_validation_requires_workspace_preparation_preflight",
        "sandbox_rendered_command_validation_requires_workspace_plan",
        "sandbox_rendered_command_validation_requires_materialization_preflight",
        "sandbox_rendered_command_validation_requires_envelope_scaffold",
        "sandbox_rendered_command_validation_requires_request_preflight",
        "sandbox_rendered_command_validation_requires_policy_matrix",
        "sandbox_rendered_command_validation_requires_known_capability",
        "sandbox_rendered_command_validation_requires_known_policy",
        "sandbox_rendered_command_validation_requires_operator_authorization",
        "sandbox_rendered_command_validation_requires_approval_lineage",
        "sandbox_rendered_command_validation_requires_final_gate",
        "sandbox_rendered_command_validation_requires_dry_run_envelope",
        "sandbox_rendered_command_validation_requires_rollback_plan",
        "sandbox_rendered_command_validation_requires_post_execution_evidence",
        "sandbox_rendered_command_validation_rejects_unknown_capability",
        "sandbox_rendered_command_validation_rejects_unknown_policy",
        "sandbox_rendered_command_validation_rejects_orphans",
        "sandbox_rendered_command_validation_rejects_stale_records",
        "source_rendered_command_scaffold_exists",
        "source_rendered_command_scaffold_fail_closed",
        "source_rendered_command_scaffold_deny_by_default",
    )
    for flag_name in required_true_flags:
        if not bool(record.get(flag_name)):
            reasons.append(f"{flag_name}_must_be_true")
    ...
    required_false_flags = (
        "sandbox_rendered_command_validation_allowed",
        "sandbox_rendered_command_validation_enabled",
        "sandbox_rendered_command_validation_performed",
        "sandbox_rendered_command_validation_passed",
        "sandbox_rendered_command_validation_failed",
        "sandbox_rendered_command_generation_allowed",
        "sandbox_rendered_command_generation_enabled",
        "sandbox_rendered_command_materialized",
        "sandbox_rendered_command_executable",
        ...
        "production_paths_mutated",
        "production_secrets_accessed",
        "source_rendered_command_generation_enabled",
        "source_rendered_command_materialized",
        "source_rendered_command_executable",
        "source_rendered_command_validated",
        "source_sandbox_execution_enabled",
        "source_result_generation_enabled",
        "source_execution_performed",
    ...
    return {
        ...
        "severity": "critical" if reasons else "info",
        "reasons": reasons,
        "subject": validation_id or rendered_scaffold_id,
        "schema_version": schema_version or "unknown",
        "sandbox_rendered_command_validation_scaffold_status": status or "unknown",
        "sandbox_rendered_command_validation_scaffold_fail_closed": bool(
            record.get("sandbox_rendered_command_validation_scaffold_fail_closed")
        ),
        "sandbox_rendered_command_validation_scaffold_deny_by_default": bool(
            record.get("sandbox_rendered_command_validation_scaffold_deny_by_default")
        ),
        "sandbox_rendered_command_validation_enabled": bool(
            record.get("sandbox_rendered_command_validation_enabled")
        ),
        "sandbox_rendered_command_validation_performed": bool(
            record.get("sandbox_rendered_command_validation_performed")
        ),
        "sandbox_rendered_command_validation_passed": bool(
            record.get("sandbox_rendered_command_validation_passed")
        ),
        "sandbox_rendered_command_validation_failed": bool(
            record.get("sandbox_rendered_command_validation_failed")
        ),
        "sandbox_rendered_command_generation_enabled": bool(
            record.get("sandbox_rendered_command_generation_enabled")
        ),
