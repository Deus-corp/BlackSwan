...
class ActionExecutor:
    ...
    async def apply(self, snapshot: SwarmSnapshot, decision: OverseerDecision, now: float) -> None:
        ...
        if decision.unblock_ips:
            await self._emit_command(
                "unblock_ips",
                self._wrap_command("sec_command", "UNBLOCK_ALL", "sec_unblock", now),
                now,
            )
        ...
        if not decision.continue_explorer:
            await self._emit_command(
                "pause_explorer",
                self._wrap_command("explorer_command", "PAUSE", "exp_pause", now),
                now,
            )
    ...
    async def _emit_command(self, key: str, command: Dict[str, Any], now: float) -> None:
        if self._is_rate_limited(key, now):
            return
    ...
    def _is_rate_limited(self, key: str, now: float) -> bool:
        last_at = self._last_emitted_at.get(key)
        if last_at is None:
            return False
        return (now - last_at) < COMMAND_COOLDOWN_SECONDS

    @staticmethod
    def _wrap_command(cmd_type: str, action: str, gid_slug: str, now: float) -> Dict[str, Any]:
        return {
            "type": cmd_type,
            "data": {"action": action},
            "timestamp": now,
