...
class StateCollector:
    ...
    def _iter_event_dicts(self, payload: Any, *, _depth: int = 0, _seen: Optional[Set[int]] = None) -> Iterable[Dict[str, Any]]:
        """Recursively traverses state to extract events."""
        if _seen is None:
            _seen = set()
    ...
    def _latest_by_node(events: Iterable[Dict[str, Any]], event_type: str) -> Dict[str, Dict[str, Any]]:
        ...
        for event in events:
            ...
            if node_id not in latest or ts >= float(latest[node_id].get("timestamp", 0.0)):
    ...
    def _recent_records(records: Iterable[Dict[str, Any]], now: float, validity: int) -> List[Dict[str, Any]]:
        ...
        return [r for r in records if now - float(r.get("timestamp", 0.0)) <= validity]
    ...
    def _count_recent(events: Iterable[Dict[str, Any]], event_type: str, now: float, validity: int) -> int:
        ...
        return sum(1 for e in events if e.get("type") == event_type and (now - float(e.get("timestamp", 0.0)) <= validity))
    ...
    def _stale_nodes(latest: Dict[str, Dict[str, Any]], now: float, threshold: int) -> List[str]:
        ...
        return [nid for nid, rec in latest.items() if now - float(rec.get("timestamp", 0.0)) > threshold]
    ...
    def _get_resource_context() -> str:
        ...
        try:
            ...
            return f"CPU: {cpu:.1f}%, RAM: {mem.percent:.1f}% ({mem.available // 1048576}MB free), Disk: {disk.percent:.1f}% ({disk.free // 1048576}MB free)"
